...
import { buildResourceForecast, forecastBalanceDirective } from "simulation/ai/petra/expertDecision/resourceForecast.js";
...
const EXPERT_FUTURE_FARM_TASK = "expertFuturePermanentFarmTask";
...
export class ExpertDecisionController
{
	constructor(HQ)
	{
		...
		// IT15.3: productive gatherers have inertia. Forecast balancing can still react,
		// but it may not repeatedly uproot whole workforces every few seconds.
		this.lastProductiveResourceRebalanceTime = -99999;
		...
		// IT14.91: four-resource forecast state.  Income/spend EMAs are intentionally
		// independent of the older food/wood continuity measurements so forecast
		// decisions can compare food, wood, stone and metal on the same footing.
		this.resourceFlowSample = undefined;
		this.resourceFlowIncomeEMA = { food: 0, wood: 0, stone: 0, metal: 0 };
		this.resourceFlowSpendEMA = { food: 0, wood: 0, stone: 0, metal: 0 };
		this.resourceFlowMeasured = false;
		this.resourceForecast = undefined;
		this.lastResourceForecastDiag = -99999;
		this.lastForecastBarterAt = -99999;
		this.lastForecastStorehouseAt = -99999;
		this.lastForecastNaturalFoodAt = -99999;
		...
		// IT14.89 field/farm-capacity progress guards.
		this.lastFieldPipeline = 0;
		...
		// IT14.96: remember a map-forced P1->P2 adaptation across saves.
		this.strategyWoodPivot = false;
		this.strategyPivotedFrom = undefined;
		this.lastStrategyWoodPivotDiag = -99999;
	...
	applyEconomicSafetyFoodOverride(gameState, accessIndex)
	{
		...
		{
			if (!ent || !ent.getMetadata || !entityPosition(ent) || !this.isExpertEconomyEntity(ent) ||
			    hasClass(ent, "Cavalry") || hasClass(ent, "CitizenSoldier") ||
			    ent.getMetadata(PlayerID, EXPERT_DEFENSE) !== undefined ||
			    ent.getMetadata(PlayerID, TASK_KEY) !== undefined || !this.attackPlanAllowsEconomicWork(gameState, ent))
				continue;
			const job = ent.getMetadata(PlayerID, JOB_METADATA);
	...
	{
		const doctrine = this.ensureStrategicDoctrine(gameState);
		if (!doctrine || this.strategyWoodPivot || !gameState.currentPhase || gameState.currentPhase() !== 1)
			return false;
		const policy = mergePolicy(this.strategyPolicyOverrides(gameState));
		const now = Number(gameState.ai.elapsedTime) || 0;
		if (now < (Number(policy.strategyWoodPivotMinimumTime) || 120))
			return false;
		const manager = this.HQ && this.HQ.attackManager;
		...
		if (!wood)
			return false;
		const bank = Math.max(0, Number(gameState.getResources().wood) || 0);
		const local = Math.max(0, Number(woodsite && woodsite.localWoodAmount) || 0);
		const income = Math.max(0, Number(woodContinuity && woodContinuity.delivered && woodContinuity.delivered.rate) || Number(wood.income) || 0);
		const accessible = Math.max(0, Number(wood.ownAccessible) || 0) + Math.max(0, Number(wood.neutralAccessible) || 0);
		const criticalStock = wood.status === "critical" &&
			(accessible <= (Number(policy.strategyWoodPivotMaximumAccessibleWood) || 900) ||
			 local <= (Number(policy.strategyWoodPivotMaximumLocalWood) || 500));
		const severeShortStock = wood.status === "short" &&
			accessible <= 0.67 * (Number(policy.strategyWoodPivotMaximumAccessibleWood) || 900) &&
			local <= 0.70 * (Number(policy.strategyWoodPivotMaximumLocalWood) || 500);
		const poorStock = criticalStock || severeShortStock;
		const poorFlow = this.woodIncomeStalled || income <= (Number(policy.strategyWoodPivotMaximumIncomeRate) || 9);
		const poorBank = bank <= (Number(policy.strategyWoodPivotMaximumWoodBank) || 350);
		if (!(poorStock && poorBank && poorFlow))
			return false;
	...
	{
		const doctrine = this.ensureStrategicDoctrine(gameState);
		let base = policyOverridesForDoctrine(doctrine, Number(gameState.ai.elapsedTime) || 0);
	...
	secondForgeResearchUseful(gameState)
	{
		...
		if (p3Boom)
		{
			// IT15.3 P3 boom does not wait for Forge #1 to become the bottleneck before
			// creating lane #2. If multiple relevant military technologies remain, build
			// the parallel lane while the economic boom can still repay it before 14:00.
			const tech = this.expertRelevantMilitaryTechStatus(gameState);
			if (tech.complete || tech.available + tech.researching < 2)
				return false;
			return (Number(res.food) || 0) >= (Number(policy.p3BoomForge2FoodBank) || 300) &&
				(Number(res.wood) || 0) >= (Number(policy.p3BoomForge2WoodBank) || 300) &&
				(Number(res.metal) || 0) >= (Number(policy.p3BoomForge2MetalBank) || 125);
		}
		// Non-P3 doctrines preserve the on-demand rule: Forge #2 earns its cost only
		// after Forge #1 is already doing useful military research.
	...
	thirdForgeResearchUseful(gameState)
	{
		...
		return (Number(res.food) || 0) >= (Number(policy.p3BoomForge3FoodBank) || 450) &&
			...
			(Number(res.metal) || 0) >= (Number(policy.p3BoomForge3MetalBank) || 175);
	}
	...
	naturalFoodSupplyWorkerLimit(gameState, supplyId, cluster)
	{
		...
		// All ordinary natural food disperses from the first command, independent of
		// Wicker: fill unused supplies first, then allow at most two workers per source.
		// Isolated large/unknown fruit trees retain the explicit three-worker exception.
		if (name.includes("apple"))
			return Math.max(1, Number(policy.naturalFoodAppleTreeMaxWorkers) || 3);
		if (name.includes("berry") || name.includes("berries"))
			return Math.max(1, Number(policy.naturalFoodMaxWorkersPerSupply) || 2);
	...
	// IT14.91: measure all four resource flows from player statistics and bank deltas.
	// spend = previous bank + newly gathered - current bank.  Barter/tribute noise is
	// intentionally smoothed by the EMA; the forecast never treats one tick as doctrine.
	measureResourceFlows(gameState)
	{
		const now = Number(gameState.ai.elapsedTime) || 0;
		const bank = gameState.getResources();
		const gathered = {};
		for (const type of ["food", "wood", "stone", "metal"])
			gathered[type] = this.playerGatheredResource(gameState, type);
		if (!this.resourceFlowSample)
		{
			this.resourceFlowSample = { time: now, bank: { food: Number(bank.food) || 0, wood: Number(bank.wood) || 0,
				stone: Number(bank.stone) || 0, metal: Number(bank.metal) || 0 }, gathered };
			return;
		}
		const dt = now - Number(this.resourceFlowSample.time || 0);
		if (dt < 4)
			return;
		let valid = false;
		for (const type of ["food", "wood", "stone", "metal"])
		{
			const currentGathered = Number(gathered[type]);
			const previousGathered = Number(this.resourceFlowSample.gathered && this.resourceFlowSample.gathered[type]);
			if (!Number.isFinite(currentGathered) || !Number.isFinite(previousGathered) || currentGathered < previousGathered)
				continue;
			valid = true;
			const gatherDelta = currentGathered - previousGathered;
			const income = Math.max(0, gatherDelta / dt);
			const previousBank = Math.max(0, Number(this.resourceFlowSample.bank && this.resourceFlowSample.bank[type]) || 0);
			const currentBank = Math.max(0, Number(bank[type]) || 0);
			const spend = Math.max(0, (previousBank + gatherDelta - currentBank) / dt);
			if (this.resourceFlowMeasured)
			{
				this.resourceFlowIncomeEMA[type] = 0.68 * (Number(this.resourceFlowIncomeEMA[type]) || 0) + 0.32 * income;
				this.resourceFlowSpendEMA[type] = 0.72 * (Number(this.resourceFlowSpendEMA[type]) || 0) + 0.28 * spend;
			}
			else
			{
				this.resourceFlowIncomeEMA[type] = income;
				this.resourceFlowSpendEMA[type] = spend;
			}
		}
		if (valid)
			this.resourceFlowMeasured = true;
		this.resourceFlowSample = { time: now, bank: { food: Number(bank.food) || 0, wood: Number(bank.wood) || 0,
			stone: Number(bank.stone) || 0, metal: Number(bank.metal) || 0 }, gathered };
	}

	queuedResourceCosts(gameState)
	{
		const total = { food: 0, wood: 0, stone: 0, metal: 0 };
		const queues = gameState && gameState.ai && gameState.ai.queues || {};
		for (const queue of Object.values(queues))
		{
			if (!queue || !Array.isArray(queue.plans))
				continue;
			// Deep production queues are intent, not an immediate bill. Count the first two
			// plans per queue so recurring production affects the forecast without exploding it.
			for (const plan of queue.plans.slice(0, 2))
			{
				if (!plan || !plan.getCost)
					continue;
				let cost;
				try { cost = plan.getCost(); } catch (e) { continue; }
				for (const type of ["food", "wood", "stone", "metal"])
					total[type] += Math.max(0, Number(cost && cost[type]) || 0);
			}
		}
		return total;
	}

	resourceForecastBaseline(gameState, foodThroughput)
	{
		const policy = mergePolicy(this.strategyPolicyOverrides(gameState));
		const phase = gameState.currentPhase ? Math.max(1, Number(gameState.currentPhase()) || 1) : 1;
		const prefix = phase >= 3 ? "P3" : phase >= 2 ? "P2" : "P1";
		const baseline = {
			food: Number(policy["resourceForecast" + prefix + "FoodSpendPerSecond"]) || 0,
			wood: Number(policy["resourceForecast" + prefix + "WoodSpendPerSecond"]) || 0,
			stone: Number(policy["resourceForecast" + prefix + "StoneSpendPerSecond"]) || 0,
			metal: Number(policy["resourceForecast" + prefix + "MetalSpendPerSecond"]) || 0
		};
		const barracks = this.builtByClass(gameState, "Barracks").length;
		const burn = barracks >= 2 ? Number(foodThroughput && foodThroughput.twoBarracksFoodBurnRate) || 0 :
			barracks === 1 ? Number(foodThroughput && foodThroughput.oneBarracksFoodBurnRate) || 0 :
			Number(foodThroughput && foodThroughput.ccFoodBurnRate) || 0;
		baseline.food = Math.max(baseline.food, burn);
		const doctrine = this.ensureStrategicDoctrine(gameState);
		if (doctrine && doctrine.id === "p2_tech_push")
		{
			baseline.wood *= 1.08;
			baseline.metal *= 1.18;
		}
		else if (doctrine && (doctrine.id === "early_p1_rush" || doctrine.id === "late_p1_rush"))
		{
			baseline.food *= 1.08;
			baseline.wood *= 1.05;
		}
	...
	resourceForecastReserves(gameState)
	{
		const policy = mergePolicy(this.strategyPolicyOverrides(gameState));
		const phase = gameState.currentPhase ? Math.max(1, Number(gameState.currentPhase()) || 1) : 1;
		const prefix = phase >= 3 ? "P3" : phase >= 2 ? "P2" : "P1";
		return {
			food: Number(policy["resourceForecastReserve" + prefix + "Food"]) || 0,
			wood: Number(policy["resourceForecastReserve" + prefix + "Wood"]) || 0,
			stone: Number(policy["resourceForecastReserve" + prefix + "Stone"]) || 0,
			metal: Number(policy["resourceForecastReserve" + prefix + "Metal"]) || 0
		};
	}

	immediatelyAccessibleResources(gameState, cc, accessIndex)
	{
		const policy = mergePolicy();
		const own = { food: 0, wood: 0, stone: 0, metal: 0 };
		const neutral = { food: 0, wood: 0, stone: 0, metal: 0 };
		const ownRadius = Math.max(20, Number(policy.resourceForecastOwnAccessibleRadius) || 55);
		const ownRadius2 = ownRadius * ownRadius;
		// IT15.1: neutral resources are expansion intelligence, not immediately-accessible
		// economic stock. The forecast must not count resources workers are forbidden to gather.
		const cap = Math.max(100, Number(policy.resourceForecastSupplyCapPerEntity) || 5000);
		const anchorsByType = {};
		for (const type of ["food", "wood", "stone", "metal"])
		{
			anchorsByType[type] = [];
			if (cc && entityPosition(cc)) anchorsByType[type].push(cc.position());
			for (const drop of this.resourceDropsites(gameState, type))
				if (entityPosition(drop)) anchorsByType[type].push(drop.position());
		}
		for (const type of ["food", "wood", "stone", "metal"])
		{
			if (!gameState.getResourceSupplies)
				continue;
			for (const supply of gameState.getResourceSupplies(type).values())
			{
				const pos = entityPosition(supply);
				if (!pos || !supply.resourceSupplyAmount || getLandAccess(gameState, supply) !== accessIndex)
					continue;
				if (type === "food" && hasClass(supply, "Field"))
					continue;
				const raw = Number(supply.resourceSupplyAmount());
				if (!Number.isFinite(raw) || raw <= 0)
					continue;
				const amount = Math.min(raw, cap);
				const owner = this.HQ.territoryMap.getOwner(pos);
				if (owner === PlayerID && anchorsByType[type].some(anchor => SquareVectorDistance(anchor, pos) <= ownRadius2))
					own[type] += amount;
			}
		}
		return { own, neutral };
	...
	updateResourceForecast(gameState, cc, accessIndex, foodThroughput)
	{
		this.measureResourceFlows(gameState);
		const policy = mergePolicy(this.strategyPolicyOverrides(gameState));
		const bank = gameState.getResources();
		const accessible = this.immediatelyAccessibleResources(gameState, cc, accessIndex);
		const forecast = buildResourceForecast({
			banks: { food: bank.food, wood: bank.wood, stone: bank.stone, metal: bank.metal },
			incomeRates: this.resourceFlowIncomeEMA,
			spendRates: this.resourceFlowSpendEMA,
			baselineSpend: this.resourceForecastBaseline(gameState, foodThroughput),
			queuedCosts: this.queuedResourceCosts(gameState),
			accessibleOwn: accessible.own,
			accessibleNeutral: accessible.neutral,
			reserves: this.resourceForecastReserves(gameState),
			minimumFloat: {
				food: policy.resourceForecastMinimumFloatFood, wood: policy.resourceForecastMinimumFloatWood,
				stone: policy.resourceForecastMinimumFloatStone, metal: policy.resourceForecastMinimumFloatMetal
			},
			shortHorizon: policy.resourceForecastShortHorizonSeconds,
			mediumHorizon: policy.resourceForecastMediumHorizonSeconds,
			floatHorizon: policy.resourceForecastFloatHorizonSeconds,
			shortAccessWeight: policy.resourceForecastShortAccessibleWeight,
			mediumAccessWeight: policy.resourceForecastMediumAccessibleWeight
		});
		this.resourceForecast = forecast;
		const now = Number(gameState.ai.elapsedTime) || 0;
		if (now - this.lastResourceForecastDiag >= (Number(policy.resourceForecastDiagnosticSeconds) || 16))
		{
			this.lastResourceForecastDiag = now;
			const target = forecast.rankedNeeds && forecast.rankedNeeds[0];
			const donor = forecast.rankedSurplus && forecast.rankedSurplus[0];
			const compact = type => {
				const r = forecast.resources[type];
				return type[0].toUpperCase() + "=" + Math.round(r.bank) + "/" + r.income.toFixed(1) + "/" +
					Math.round(r.accessible) + "/" + Math.round(r.mediumDemand) + "/L" + Math.round(r.liquidMediumMargin) +
					"/S" + Math.round(r.mediumMargin) + ":" + r.status;
			};
			aiWarn("[EXPERT-FORECAST] target=" + (target ? target.type + ":" + target.status : "none") +
				" donor=" + (donor ? donor.type + ":" + donor.status : "none") + " " +
				["food", "wood", "stone", "metal"].map(compact).join(" "));
		}
		return forecast;
	}

	forecastDirective(allowedTargets)
	{
		return forecastBalanceDirective(this.resourceForecast, allowedTargets);
	}

	forecastResourceCritical(type)
	{
		const resource = this.resourceForecast && this.resourceForecast.resources && this.resourceForecast.resources[type];
		return !!(resource && resource.status === "critical");
	}
	...
	foodClusterAssignmentServiced(gameState, cluster)
	{
		if (!cluster || !cluster.center)
			return false;
		const comfort = Number(mergePolicy().naturalFoodDropsiteComfortDistance) || 15;
		const servicePoints = (cluster.ids || []).map(id => gameState.getEntityById(Number(id)))
			.map(entityPosition).filter(Boolean);
		if (!servicePoints.length)
			servicePoints.push(cluster.center);
		const serves = position => servicePoints.some(point =>
			SquareVectorDistance(position, point) <= comfort * comfort);
		// Gathering may begin beside a completed dropsite, or once the dedicated
		// Farmstead foundation has actually been placed. A queued/pending Farmstead is
		// not service: sending civilians before the foundation exists recreates the
		// long-haul berry walk this gate is intended to prevent.
		for (const dropsite of this.foodDropsites(gameState))
			if (serves(dropsite.position()))
				return true;
		for (const foundation of this.foundationsByClass(gameState, "Farmstead"))
			if (entityPosition(foundation) && serves(foundation.position()))
				return true;
		return false;
	}
	...
	applyPostWickerBranchConstruction(gameState, frame)
	{
		if (!this.postWickerBranchFarmsteadPending || !this.postWickerBranchCluster || !this.postWickerBranchWorkerIds.length)
			return frame;
		const policy = mergePolicy();
		const firstBarracksPlaced = this.builtByClass(gameState, "Barracks").length > 0 ||
			this.foundationsByClass(gameState, "Barracks").length > 0;
		if (!firstBarracksPlaced)
		{
			// Barracks #1 owns the opening wood first. Keep the peeled civilians on
			// their local productive jobs and start the Farmstead placement clock only
			// after the Barracks foundation exists.
			this.postWickerBranchFarmsteadStartedAt = Number(gameState.ai.elapsedTime) || 0;
			return frame;
		}
	...
	researchExpertEcoTech(gameState, queues, foodClusters, cc)
	{
		...
		// IT15.8.17: multiple palms/bushes inside one connected opening patch are still
		// one natural-food source. Wicker requires a second worthwhile owned cluster.
		const wickerPriority = multipleFruit;
		...
		const naturalTransitionReached = !Number.isFinite(Number(this.lastTerritoryNaturalFoodRatio)) ||
			Number(this.lastTerritoryNaturalFoodRatio) <= Number(policy.territoryNaturalFarmTransitionRatio || 0.25);
		if (farmStarted && naturalTransitionReached && !gameState.isResearched(plows) && !gameState.isResearching(plows) && available.has(plows))
		{
	...
	researchExpertSurplusEcoParallel(gameState, queues)
	{
		// The food/wood core lanes may be busy while a large bank can buy a
		// separate, useful mining/carry upgrade. Keep this to one additional lane.
		if (!gameState.currentPhase || gameState.currentPhase() < 2 || !gameState.ai.queueManager)
			return false;
		const bank = gameState.getResources();
		if (bank.food < 1200 || bank.wood < 600 || bank.stone < 250 || bank.metal < 250)
			return false;
		const lane = "expertSurplusEcoParallel";
		gameState.ai.queueManager.addQueue(lane, 790);
		...
		for (const q of Object.values(queues))
			for (const plan of q && q.plans || [])
				if (plan && plan.type)
					already.add(plan.type);
		const workers = this.economyWorkerMetrics(gameState);
		const options = [];
		for (const [name, data] of gameState.findAvailableTech() || [])
		{
			if (!name || already.has(name) || gameState.isResearched(name) || gameState.isResearching(name) ||
			...
			const mining = values.some(value => value.includes("stone.rock") || value.includes("metal.ore"));
			const carrying = values.some(value => value.startsWith("ResourceGatherer/Capacities"));
			if ((!mining || (Number(workers.stone) || 0) + (Number(workers.metal) || 0) < 3) && !carrying)
				continue;
			const cost = data._template.cost || {};
			if (bank.food < (Number(cost.food) || 0) + 500 || bank.wood < (Number(cost.wood) || 0) + 250 ||
			    bank.stone < (Number(cost.stone) || 0) + 100 || bank.metal < (Number(cost.metal) || 0) + 100)
				continue;
			options.push({ name, score: (mining ? (Number(workers.stone) || 0) + (Number(workers.metal) || 0) : 0) * 100 -
				(Number(cost.food) || 0) - (Number(cost.wood) || 0) - (Number(cost.stone) || 0) - (Number(cost.metal) || 0) });
		}
		options.sort((a, b) => b.score - a.score || a.name.localeCompare(b.name));
		if (!options.length)
			return false;
		const plan = new ResearchPlan(gameState, options[0].name, false);
		plan.metadata = { "expertDecisionLayer": true, "expertEcoTech": "parallel-surplus" };
		queue.addPlan(plan);
		aiWarn("[EXPERT-TECH] queued parallel surplus=" + options[0].name +
			" bank=" + Math.round(bank.food) + "/" + Math.round(bank.wood) + "/" +
	...
	researchExpertP1EcoSweep(gameState, queues)
	{
		...
		for (const tech of gameState.findAvailableTech() || [])
		{
			...
			if (name === "gather_farming_plows" && Number.isFinite(Number(this.lastTerritoryNaturalFoodRatio)) &&
			    Number(this.lastTerritoryNaturalFoodRatio) > Number(policy.territoryNaturalFarmTransitionRatio || 0.25))
				continue;
	...
	expertCombatTrainingOwner(gameState)
	{
		...
		if (started.length)
		{
			const primary = started.sort((a, b) => (b.unitCollection ? b.unitCollection.length : 0) - (a.unitCollection ? a.unitCollection.length : 0))[0];
			// IT15.3 P1 pressure doctrine: after the rush launches, fresh citizen-soldiers
			// work the home economy instead of trickling 1-3 at a time across the map.
			// Wounded-replacement logic may still send a deliberate batch when the field
			// army actually needs replacements; otherwise a failed push leaves us ahead
			// economically for the retreat/boom/tech transition.
			if (primary.type === AttackPlan.TYPE_RUSH)
				return undefined;
			return primary;
		}
	...
	updateExpertCombatAuthority(gameState)
	{
		...
		const finish = this.finishingState(gameState);
		if (finish.active)
		{
			const live = this.expertCombatPlans(true).filter(plan => plan && plan.expertAuthorityOwned &&
				plan.targetPlayer === finish.targetPlayer && plan.unitCollection);
			live.sort((a, b) => b.unitCollection.length - a.unitCollection.length);
			const plan = live[0];
			if (plan && plan.unitCollection.length < 80)
			{
				const reserve = [];
				for (const ent of gameState.getOwnUnits().values())
					if (ent && entityPosition(ent) && ent.getMetadata &&
					    (hasClass(ent, "CitizenSoldier") || hasClass(ent, "Champion")) &&
					    !hasClass(ent, "Support") && !isExpertBuildingSiegeEntity(ent) &&
					    (ent.getMetadata(PlayerID, "plan") === undefined || ent.getMetadata(PlayerID, "plan") === -1) &&
					    ent.getMetadata(PlayerID, EXPERT_DEFENSE) === undefined &&
				...
				if (reserve.length >= 20)
				{
					...
					let added = 0;
					const cap = Math.min(reserve.length - 4, 80 - plan.unitCollection.length);
					for (const ent of reserve.slice(0, Math.max(0, cap)))
						if (this.expertAttachEntityToPlan(plan, ent))
							++added;
					if (added)
							aiWarn("[EXPERT-FINISH] rallied reserve=" + added + " plan=" + plan.name +
								" army=" + plan.unitCollection.length + " enemyPop=" + finish.enemyPopulation);
				}
			}
		}
	...
	expertRelevantMilitaryTechStatus(gameState)
	{
		...
		for (const qName of ["expertMilitaryTech1", "expertMilitaryTech2", "expertMilitaryTech3", "expertAthensP1Melee", "expertHopliteTradition"])
		{
	...
	researchExpertP2MilitaryTech(gameState, queues)
	{
		...
		if (!p3AllIn && militaryProgress.queued >= policy.expertP2MilitaryTechsBeforeSecondEcoPair && ecoProgress.queued < 4 && !warSurplus)
			return false;
		const queueManager = gameState.ai.queueManager;
		const laneNames = ["expertMilitaryTech1", "expertMilitaryTech2", "expertMilitaryTech3"].slice(0, Math.min(3, forgeCount));
		...
		for (const tech of availableTechs)
		{
			...
			if (!p3AllIn && !forcedAthensCore && pureCavalry && cavalryArmy < 4)
				continue;
			if (!p3AllIn && !forcedAthensCore && pureJavelineer && javelineerArmy < Math.max(6, Math.ceil((meleeArmy + rangedArmy) * 0.15)))
				continue;
		...
		for (const qName of laneNames)
		{
			...
			queueManager.changePriority(qName, 780 - Math.max(0, laneNames.indexOf(qName)) * 5);
			remaining.food -= pick.cost.food; remaining.wood -= pick.cost.wood;
			remaining.stone -= pick.cost.stone; remaining.metal -= pick.cost.metal;
			++queued;
	...
	coordinateCivilianSafety(gameState, cc)
	{
		...
		for (const enemy of gameState.getEnemyUnits().values())
			if (enemy && entityPosition(enemy) && this.isCombatUnit(enemy))
				enemies.push(enemy);
		// IT15.0: Expert owns the release side of its own civilian shelter decision.
		// Do not rely on the generic garrison manager's different threat thresholds.
		for (const holder of gameState.getOwnStructures().values())
		{
			if (!holder || !holder.garrisoned || !holder.unload || !entityPosition(holder) ||
			    (!hasClass(holder, "House") && !hasClass(holder, "CivCentre")))
				continue;
			let holderThreatened = false;
			for (const enemy of enemies)
				if (SquareVectorDistance(holder.position(), enemy.position()) <=
				    policy.civilianDangerRadius * policy.civilianDangerRadius)
				{
					holderThreatened = true;
					break;
				}
			if (holderThreatened)
				continue;
			for (const id of [...(holder.garrisoned() || [])])
			{
				const civilian = gameState.getEntityById(Number(id));
				if (!civilian || !hasClass(civilian, "Civilian") || hasClass(civilian, "CitizenSoldier") ||
				    civilian.getMetadata(PlayerID, EXPERT_CIVILIAN_EVAC) === undefined)
					continue;
				const last = Number(civilian.getMetadata(PlayerID, EXPERT_CIVILIAN_DANGER_AT));
				if (!Number.isFinite(last) || now - last < policy.civilianEvacuationReleaseSeconds)
					continue;
				holder.unload(id);
				aiWarn("[EXPERT-CIV] ungarrison worker=" + id + " holder=" + holder.id() + " reason=threat-clear");
			}
		}
		...
		for (const ent of gameState.getOwnUnits().values())
		{
			...
			const pos = entityPosition(ent);
			if (!pos)
				continue; // still garrisoned/transitioning; explicit release is handled above.
	...
	rescanExistingFarmCapacityBeforeThirdHub(gameState, accessIndex, farmCapacity)
	{
		const policy = mergePolicy();
		const now = Number(gameState.ai.elapsedTime) || 0;
	...
	applyPreThirdFarmsteadCapacityRescan(gameState, frame, accessIndex)
	{
	...
	applyPermanentFieldProgressWatchdog(gameState, frame, accessIndex, foodNetwork)
	{
		...
		const pipeline = built + foundations + queued;
		let idleFoodDebt = 0;
		for (const ent of gameState.getOwnUnits().values())
		{
			if (!ent || !ent.getMetadata || !hasClass(ent, "Civilian") || hasClass(ent, "CitizenSoldier") ||
			    !(ent.isIdle && ent.isIdle()) || ent.getMetadata(PlayerID, TASK_KEY) !== undefined ||
			    ent.getMetadata(PlayerID, EXPERT_DEFENSE) !== undefined || ent.getMetadata(PlayerID, EXPERT_CIVILIAN_EVAC) !== undefined)
				continue;
			const job = ent.getMetadata(PlayerID, JOB_METADATA);
			if (job === "food" || job === "food_owned" || job === "farm" || ent.getMetadata(PlayerID, EXPERT_FUTURE_FARMER) === true)
				++idleFoodDebt;
		}
		const baseDesired = Math.max(0, Number(economyDesired !== undefined && economyDesired !== null ? economyDesired : fallbackDesired) || 0);
		const crew = Math.max(1, Number(frame.economy && frame.economy.derived && frame.economy.derived.preferredFarmersPerField) || 4);
		const natural = frame.state && frame.state.food || {};
		const naturalRemaining = Math.max(0, Number(natural.totalNaturalRemaining) || 0);
		...
		const idleFieldDebt = !healthyNatural ? idleFoodDebt : 0;
		const desired = Math.max(healthyNatural ? pipeline : baseDesired, idleFieldDebt > 0 ? pipeline + Math.ceil(idleFieldDebt / crew) : 0);
		if (pipeline > Number(this.lastFieldPipeline || 0) || this.lastFieldPipelineProgressAt < -90000)
			this.lastFieldPipelineProgressAt = now;
		this.lastFieldPipeline = pipeline;
		if (desired <= pipeline)
		{
			this.lastFieldPipelineProgressAt = now;
			return frame;
		}
		const deficitSeconds = Math.max(0, Number(natural.foodInfrastructureDeficitSeconds) || 0);
		const foodBank = Math.max(0, Number(gameState.getResources().food) || 0);
		const pressure = idleFieldDebt > 0 ||
			(!healthyNatural && naturalRemaining <= (Number(policy.fieldProgressWatchdogNaturalRemaining) || 120)) ||
			deficitSeconds >= (Number(policy.fieldProgressWatchdogFoodDeficitSeconds) || 12) || foodBank <= 300;
		const forecastFood = this.resourceForecast && this.resourceForecast.resources && this.resourceForecast.resources.food;
		let readyFutureFarmers = 0;
		for (const ent of gameState.getOwnUnits().values())
			if (ent && ent.getMetadata && ent.getMetadata(PlayerID, EXPERT_FUTURE_FARMER) === true &&
			    ent.getMetadata(PlayerID, EXPERT_FUTURE_FARM_TASK) === undefined && !Number.isFinite(Number(ent.getMetadata(PlayerID, FARM_LOCK))) &&
			    ent.getMetadata(PlayerID, TASK_KEY) === undefined)
				++readyFutureFarmers;
		const forecastFieldPressure = !!(forecastFood && (forecastFood.status === "critical" || forecastFood.status === "short") &&
			readyFutureFarmers >= Math.max(2, Number(policy.futureFarmFieldBuilderMinimum) || 3));
		const watchdogDelay = idleFieldDebt > 0 ? 0 : forecastFieldPressure ? 6 : (Number(policy.fieldProgressWatchdogSeconds) || 24);
		if (!pressure || now - this.lastFieldPipelineProgressAt < watchdogDelay)
			return frame;
		if ((frame.actions || []).some(action => action && action.kind === "field" && (action.type === "BUILD" || action.type === "RESERVE")))
			return frame;
		const capacity = this.farmCapacitySnapshot(gameState, accessIndex);
		if (!capacity.known || Number(capacity.openFieldSlots) <= 0)
			return frame;
		const action = { "type": "BUILD", "kind": "field", "role": "field_progress_watchdog",
			"priority": Number(policy.fieldProgressWatchdogPriority) || 127, "builderCount": 3,
			"builderPool": ["food", "food_owned", "farm"] };
		if (now - (Number(this.lastFieldWatchdogDiag) || -99999) >= 8)
		{
			this.lastFieldWatchdogDiag = now;
			aiWarn("[EXPERT-FIELD-WATCH] force next field pipeline=" + pipeline + "/" + desired +
				" open=" + Number(capacity.openFieldSlots || 0) + " food=" + Math.round(foodBank) +
				" natural=" + Math.round(naturalRemaining) + " ratio=" + naturalRatio.toFixed(2) + " slots=" + naturalSlots + " idleFoodDebt=" + idleFoodDebt +
				" stalled=" + Math.round(now - this.lastFieldPipelineProgressAt) + "s");
		}
	...
	applyFutureFarmerFieldCrews(gameState, frame)
	{
		if (!frame || !Array.isArray(frame.actions))
			return frame;
		const policy = mergePolicy();
		const target = Math.max(1, Number(policy.futureFarmFieldBuilderTarget) || 4);
		const normalMinimum = Math.max(1, Math.min(target, Number(policy.futureFarmFieldBuilderMinimum) || 3));
		const emergencyMinimum = Math.max(1, Math.min(normalMinimum, Number(policy.futureFarmFieldBuilderEmergencyMinimum) || 2));
		const bank = gameState.getResources();
		const future = [];
		for (const ent of gameState.getOwnUnits().values())
		{
			if (!ent || !entityPosition(ent) || !ent.getMetadata || !hasClass(ent, "Civilian") ||
			    ...
			    (this.workerActualResource(gameState, ent) === "food" &&
			     ent.getMetadata(PlayerID, NATURAL_FOOD_LOCK) !== undefined))
				continue;
			future.push(ent);
		}
		const fieldBuildCount = frame.actions.filter(action => action && action.kind === "field" && action.type === "BUILD").length;
		const foodForecast = this.resourceForecast && this.resourceForecast.resources && this.resourceForecast.resources.food;
		const naturalRemaining = frame.state && frame.state.food ? Math.max(0, Number(frame.state.food.totalNaturalRemaining) || 0) : 0;
		const emergencyFieldRecovery = fieldBuildCount > 0 && naturalRemaining <=
			(Math.max(0, Number(policy.fieldProgressWatchdogNaturalRemaining) || 120)) && foodForecast &&
			(foodForecast.status === "critical" || foodForecast.status === "short");
		// Recover a normal three-person crew first even in an emergency.  The two-person
		// minimum is only a last-resort launch floor if the map truly cannot provide three.
		// This prevents the watchdog from repeatedly creating a Field action that the crew
		// filter then discards for lack of its own builders.
		const neededPool = fieldBuildCount * normalMinimum;
		if (future.length < neededPool)
		{
			...
			for (const ent of gameState.getOwnUnits().values())
			{
				...
				const actualForecast = actual && this.resourceForecast && this.resourceForecast.resources[actual];
				// Never steal from a currently-short resource. Prefer food-owned civilians
				// temporarily sitting on a surplus mine/forest: those are exactly the CC-trained
				// future farmers that 14.91 lost behind metadata.
				if (!emergencyFieldRecovery && actualForecast && (actualForecast.status === "critical" || actualForecast.status === "short") && actual !== "food")
					continue;
				const foodOwned = current === "food_owned" || current === "food";
				const surplusActual = !!(actualForecast && (actualForecast.status === "surplus" || actualForecast.coverageRatio >= 1.5));
				candidates.push({ ent, ordinal, foodOwned, surplusActual, actual: actual || jobResourceType(current) || "idle" });
			}
			candidates.sort((a, b) => Number(b.foodOwned) - Number(a.foodOwned) || Number(b.surplusActual) - Number(a.surplusActual) || b.ordinal - a.ordinal);
			for (const item of candidates)
			{
				if (future.length >= neededPool) break;
				item.ent.setMetadata(PlayerID, EXPERT_FUTURE_FARMER, true);
				future.push(item.ent);
				aiWarn("[EXPERT-FARM-CREW] recover future-farmer=" + item.ent.id() + " from=" + item.actual + " ordinal=" + item.ordinal);
			}
		}
		future.sort((a, b) => {
			const oa = Number(a.getMetadata(PlayerID, CIVILIAN_ORDINAL)) || 99999;
			const ob = Number(b.getMetadata(PlayerID, CIVILIAN_ORDINAL)) || 99999;
		...
		{
			if (!action || action.kind !== "field" || action.type !== "BUILD")
			{
				actions.push(action);
				continue;
			}
			const natural = frame.state && frame.state.food ? frame.state.food : {};
			const emergency = emergencyFieldRecovery || ((Number(bank.food) || 0) <= (Number(policy.futureFarmFieldEmergencyFoodBank) || 120) &&
			...
			const selected = future.slice(cursor, cursor + count);
			cursor += count;
			actions.push({ ...action,
				"builderCount": count,
				"requiredBuilderIds": selected.map(ent => ent.id()),
				"builderPool": ["food", "food_owned", "farm"],
				"builderJobPriority": { "farm": 8, "food_owned": 7, "food": 6 }
			});
			if ((Number(gameState.ai.elapsedTime) || 0) - (Number(this.lastFutureFarmCrewDiag) || -99999) >= 6)
			{
				this.lastFutureFarmCrewDiag = Number(gameState.ai.elapsedTime) || 0;
				aiWarn("[EXPERT-FARM-CREW] field role=" + (action.role || "field") +
					" futureBuilders=" + selected.map(ent => ent.id()).join(",") + " stay=farmer");
			}
		}
		return { ...frame, actions };
	}

	applyPhase2SafetyField(gameState, frame, farmCapacity, accessIndex)
	{
		const policy = mergePolicy(this.strategyPolicyOverrides(gameState));
		...
		if (naturalRemaining > 0 && naturalRatio > Number(policy.territoryNaturalFarmTransitionRatio || 0.25))
			return frame;
		...
		let open = farmCapacity && farmCapacity.known ? Math.max(0, Number(farmCapacity.openFieldSlots) || 0) : 0;
		const now = Number(gameState.ai.elapsedTime) || 0;
		...
		if (open <= 0)
		{
			...
			if (farmsteadPipeline >= 1 && farmsteadPipeline < Math.max(1, Number(policy.maximumFarmsteads) || 3))
			{
				...
				if (open > 0)
				{
					const existingField = actions.find(action => action && action.kind === "field");
					if (existingField) existingField.priority = Math.max(Number(existingField.priority) || 0, Number(policy.phase2SafetyFieldPriority) || 125);
					else actions.push({ "type": "BUILD", "kind": "field", "role": "pre_third_hub_repack",
						"priority": Number(policy.phase2SafetyFieldPriority) || 125, "builderCount": Number(policy.phase2SafetyFieldBuilders) || 2,
						"builderPool": ["food", "food_owned", "farm"] });
					return { ...frame, actions };
				}
			}
	...
	phase2Readiness(gameState, frame)
	{
		...
		const naturalTransitionHealthy = naturalRemaining > 0 && naturalRatio > Number(policy.territoryNaturalFarmTransitionRatio || 0.25);
		...
		const woodScarcityPhaseEscape = this.strategyWoodPivot && barracks >= 1 &&
			(fieldPipeline >= (Number(policy.strategyWoodPivotPhase2MinimumFields) || 4) || naturalInfrastructureHealthy) &&
			now >= (Number(policy.strategyWoodPivotPhase2MinimumTime) || 300) &&
			pop >= (Number(policy.strategyWoodPivotPhase2MinimumPopulation) || 55) &&
			coverage >= (Number(policy.strategyWoodPivotPhase2CostCoverage) || 0.65);
		...
		// IT14.74: healthy >25% combined natural food substitutes for the old six-Field
		...
		else if (woodScarcityPhaseEscape)
			ready = true, lane = "wood-scarcity-pivot";
	...
	updateWoodContinuityWatchdog(gameState, workers, woodsite)
	{
		...
		const forecastComfortable = !!(forecastWood && (forecastWood.status === "surplus" || forecastWood.status === "balanced") &&
			forecastWood.bank >= Math.max(Number(forecastWood.dynamicCeiling) || 0, Number(policy.resourceJobEmergencyWoodBank) || 250));
		// IT14.91: zero current lumber deliveries are not a crisis while the bank plus
		// accessible stock already covers projected spending. This removes the Arctic
		// "7k wood but WOOD STALLED" false alarm without weakening true low-runway cases.
		this.woodIncomeStalled = !forecastComfortable && !!(hardZeroStall || (delivered.measured && enoughWorkers && lowOrNoActiveWood &&
	...
	ensureInitialWoodSelection(gameState, cc, accessIndex)
	{
		if (this.initialWoodSelection && this.initialWoodSelection.position)
			return;
		const policy = mergePolicy();
		// IT14.97: "best opening forest" means the best same-land forest we can
		// economically service from a legal Storehouse in our territory. IT14.93's
		// selector was still fed OWN trees only, so sparse maps could pick a 300-500
		// IT15.8.11: score a safely serviceable neutral border forest too. The
		// Storehouse itself must still pass the owned-territory legality test below.
		const collect = allowNeutralWood => collectInitialWoodCandidates(gameState, {
			"getLandAccess": getLandAccess,
		...
		const choose = candidates => selectInitialWoodWorksite(candidates, cc.position(), {
			...
			"continuationLinkDistance": Number(policy.openingWoodContinuationLinkDistance) || 28,
			"continuationReach": Number(policy.openingWoodContinuationReach) || 110,
			"continuationWeight": Number(policy.openingWoodContinuationWeight) || 0.65
		});
		let selection = choose(trees);
		const legalRadius = Math.max(12, Number(policy.openingWoodLegalPlacementRadius) || 32);
		const legalDistances = [];
		for (let distance = 0; distance <= legalRadius + 0.001; distance += 4)
			legalDistances.push(distance);
		const treeById = new Map(trees.map(tree => [Number(tree.id), tree]));
		const annotate = site =>
		{
			if (!site || !Array.isArray(site.position))
				return undefined;
			const placementCandidates = initialStorehousePlacementCandidates(
				{ "action": "SELECT_INITIAL_WOODSITE", ...site },
				{ "distances": legalDistances, "angleCount": 32 });
			let legalPosition, legalOffset = Infinity;
			for (const position of placementCandidates)
			{
				if (this.HQ.territoryMap.getOwner(position) !== PlayerID ||
				    gameState.ai.accessibility.getAccessValue(position) !== accessIndex)
					continue;
				const offset = Math.sqrt(SquareVectorDistance(site.position, position));
			...
			if (!legalPosition)
				return undefined;
			let ownWood = 0, neutralWood = 0;
			for (const id of site.forestTreeIds && site.forestTreeIds.length ? site.forestTreeIds : site.treeIds || [])
			{
				const tree = treeById.get(Number(id));
				if (!tree) continue;
				if (Number(tree.territoryOwner) === 0) neutralWood += Math.max(0, Number(tree.remaining) || 0);
				else if (Number(tree.territoryOwner) === PlayerID) ownWood += Math.max(0, Number(tree.remaining) || 0);
			}
			const minimum = Math.max(1, Number(policy.openingWoodMinimumSustainableAmount) || 800);
			return { ...site, legalStorehousePosition: [...legalPosition], legalStorehouseOffset: legalOffset,
				ownWoodAmount: ownWood, neutralWoodAmount: neutralWood,
				sustainable: Number(site.forestWoodAmount || site.localWoodAmount) >= minimum,
				effectiveOpeningScore: (Number(site.score) || 0) - legalOffset * (Number(policy.openingWoodLegalOffsetWeight) || 6) };
		};

		let ranked = selection && selection.position ? (selection.ranked && selection.ranked.length ? selection.ranked : [selection])
			.map(annotate).filter(Boolean) : [];
		if (!ranked.length)
		{
			this.initialWoodSelection = selection;
			return;
		}

		const sustainable = ranked.filter(site => Number(site.forestWoodAmount || site.localWoodAmount) >=
			Math.max(1, Number(policy.openingWoodMinimumSustainableAmount) || 800));
		const pool = sustainable.length ? sustainable : ranked;
		pool.sort((a, b) => b.effectiveOpeningScore - a.effectiveOpeningScore ||
			b.localWoodAmount - a.localWoodAmount || a.approachDistance - b.approachDistance);
		let selected = pool[0];
		const richest = [...pool].sort((a, b) =>
			Number(b.forestWoodAmount || b.localWoodAmount) - Number(a.forestWoodAmount || a.localWoodAmount) ||
			b.localWoodAmount - a.localWoodAmount || a.approachDistance - b.approachDistance)[0];
		const dominanceRatio = Math.max(1, Number(policy.openingWoodDominanceRatio) || 1.75);
		if (richest && selected && richest !== selected &&
		    Number(richest.forestWoodAmount || richest.localWoodAmount) >= Number(selected.forestWoodAmount || selected.localWoodAmount) * dominanceRatio &&
		    richest.approachDistance <= (Number(policy.openingWoodDominanceMaxApproach) || 170) &&
		    richest.approachDistance - selected.approachDistance <= (Number(policy.openingWoodDominanceMaxExtraApproach) || 90))
			selected = richest;
		const ordered = [selected, ...ranked.filter(site => site !== selected).sort((a, b) =>
			(Number(b.sustainable) - Number(a.sustainable)) || b.effectiveOpeningScore - a.effectiveOpeningScore ||
			b.localWoodAmount - a.localWoodAmount || a.approachDistance - b.approachDistance)];
		this.initialWoodSelection = { ...selected, ranked: ordered };
		aiWarn("[EXPERT-WOOD-OPEN] selected mode=" + (selected.sustainable ? "sustainable" : "bridge") +
			" wood=" + Math.round(Number(selected.localWoodAmount) || 0) +
			" forest=" + Math.round(Number(selected.forestWoodAmount || selected.localWoodAmount) || 0) +
			" continuation=" + Math.round(Number(selected.continuationWoodAmount) || 0) +
			" own=" + Math.round(Number(selected.ownWoodAmount) || 0) +
			" neutral=" + Math.round(Number(selected.neutralWoodAmount) || 0) +
	...
	{
		if (!gameState || !this.isCityStateCiv(gameState) || !gameState.currentPhase || gameState.currentPhase() !== 1)
			return frame;
		const doctrine = this.ensureStrategicDoctrine(gameState);
	...
	syncJobs(gameState, foodNetwork, foodThroughput)
	{
		...
		const soldierBalance = balancingActive && this.resourceForecast ? this.forecastDirective(soldierTargets) :
			balancingActive ? resourceBalanceDirective({ ...balanceInput, "allowedTargets": soldierTargets }) : { "active": false };
		...
		let futureFarmLabor = farmWorkers + civilians.filter(ent => ent.getMetadata(PlayerID, EXPERT_FUTURE_FARMER) === true &&
			ent.getMetadata(PlayerID, EXPERT_FUTURE_FARM_TASK) === undefined && !Number.isFinite(Number(ent.getMetadata(PlayerID, FARM_LOCK)))).length;
		...
		for (const entry of reconciled.civilians)
		{
			...
			// permanent six-Field backbone exists. Every other dedicated civilian is
			// food-owned regardless of a stale "permanent" wood/stone/metal job inherited
			// from an earlier decision tick. Citizen-soldiers own the secondary economy.
			const openingLumberCivilian = entry.ordinal > policy.startingNaturalFoodCivilians &&
				entry.ordinal <= policy.startingNaturalFoodCivilians + policy.firstTrainedWoodCivilians;
			if (rushFoodCivilianDoctrine && (!openingLumberCivilian || fields >= 6))
			{
				if (entry.ordinal > openingEnd && ent.getMetadata(PlayerID, EXPERT_FUTURE_FARMER) !== true)
					ent.setMetadata(PlayerID, EXPERT_FUTURE_FARMER, true);
				if (current !== "food" && current !== "food_owned" && current !== "farm")
				{
					if (current === "wood")
					...
					this.setDesiredJob(gameState, ent, "food_owned", { "force": true });
					++foodWorkers;
					aiWarn("[EXPERT-FOOD-DOCTRINE] civilian=" + ent.id() + " " + (current || "none") +
						"->food_owned ordinal=" + entry.ordinal + " fields=" + fields);
				}
				continue;
			}

			// IT15.8.12 normal first-Barracks handoff: do not finish the old 20-civilian
			// lumber script while the permanent food engine is missing. Only civilians
			// that have not yet received a permanent job enter this path, so the original
			// berry crew finishes its natural sources and existing lumberjacks stay put.
			if (barracksFoodHandoff && !hadPermanentJob)
			{
				if (ent.getMetadata(PlayerID, EXPERT_FUTURE_FARMER) !== true)
				{
					ent.setMetadata(PlayerID, EXPERT_FUTURE_FARMER, true);
				...
				this.setDesiredJob(gameState, ent, "food_owned", { "force": true });
				++foodWorkers;
				aiWarn("[EXPERT-FARM-HANDOFF] new civilian=" + ent.id() +
					" reserved=future-farmer fields=" + fields + " target=6");
				continue;
			}

			if (entry.ordinal <= openingEnd)
			{
				// IT15.0 Steven-derived handoff: once six permanent Fields are online in a
				// rush build, even the three trained opening lumber civilians become
				// food-owned. Citizen-soldiers inherit the secondary-resource economy.
				const rushFoodBackbone = Number(policy.targetWoodCivilians) <= 3 && fields >= 6 &&
					entry.ordinal > policy.startingNaturalFoodCivilians &&
					entry.ordinal <= policy.startingNaturalFoodCivilians + policy.firstTrainedWoodCivilians;
				if (rushFoodBackbone)
				{
					desired = farmWorkers < fields * preferredFarmersPerField ? "farm" : "food_owned";
					// This is a doctrine handoff, not ordinary resource balancing. The old
					// permanent-job lease must not veto moving the three opening lumber
					// civilians onto the completed food backbone.
					if (jobResourceType(current) === "wood")
					{
						ent.setMetadata(PlayerID, EXPERT_JOB_LEASE_UNTIL, undefined);
						ent.setMetadata(PlayerID, EXPERT_JOB_LEASE_RESOURCE, undefined);
						...
						if (!hasClass(ent, "Cavalry"))
							ent.setMetadata(PlayerID, "subrole", Worker.SUBROLE_IDLE);
						aiWarn("[EXPERT-FOOD-BACKBONE] civilian=" + ent.id() + " wood->" + desired +
							" fields=" + fields + " doctrine=" + (this.ensureStrategicDoctrine(gameState).id || "rush"));
					}
				}
				...
				if (!desired && ent.getMetadata(PlayerID, EXPERT_ADAPTIVE_FOOD) === true)
					desired = "food_owned";
				else if (!desired && ent.getMetadata(PlayerID, EXPERT_WICKER_BRANCH) === true && ent.getMetadata(PlayerID, NATURAL_FOOD_LOCK))
					desired = "food_owned";
				else if (!desired && ent.getMetadata(PlayerID, EXPERT_WICKER_BRANCH) === true && this.postWickerBranchFarmsteadPending)
					desired = "food";
				else if (!desired && ent.getMetadata(PlayerID, EXPERT_WICKER_PEELED) === true)
					desired = "wood";
			...
			{
				// IT14.90: after the scripted ~20 civilian wood tranche, NEW civilians
				// are the permanent farm-expansion labor. We keep the normal food_owned
				// job so they can gather natural food or work temporary overflow while
				// waiting, but the metadata flag makes them the exclusive normal Field
				    ...
				    futureFarmLabor < futureFarmTargetWorkers)
				{
					...
					++futureFarmLabor;
					desired = "food_owned";
					aiWarn("[EXPERT-FARM-CREW] new civilian=" + ent.id() +
						" reserved=future-farmer foodLabor=" + foodWorkers + "/" + futureFarmTargetWorkers);
				}
				...
				if (["wood", "food", "food_owned", "farm", "stone", "metal"].includes(current) && !idleWithFoodCapacity)
					continue;
				if (!desired && idleWithFoodCapacity)
				{
					desired = "farm";
				...
				if (mustFeed)
				{
					const d = decidePostOpeningCivilianJob({
						...
						"farmersPerField": preferredFarmersPerField, "targetWoodCivilians": policy.targetWoodCivilians, "postOpeningFoodFloor": policy.postOpeningFoodFloor,
				...
				else if (!desired && civilianBalance.active)
				{
					...
					aiWarn("[EXPERT-BALANCE] new civilian=" + ent.id() + " -> " + civilianBalance.target + " ratio=" + Number(civilianBalance.ratio || 1).toFixed(2));
				}
				else if (!desired)
				{
					const d = decidePostOpeningCivilianJob({
		...
		// IT14.92: one economic authority after the protected opening. The forecast
		// chooses the shortage and donor; old food-vs-wood/mining peelers must not issue
		// contradictory cross-resource orders in the same update. Before the adaptive
		// window starts, retain the proven opening safety nets unchanged.
		if (this.resourceForecast && gameState.ai.elapsedTime >= policy.resourceBalanceStartTime)
	...
	// food emergencies now preserve military/building ownership and use civilians,
	// infrastructure, or barter instead.
	applyProFoodBankRebalance(gameState, feedback)
	{
		...
		// Food emergencies are solved by civilians, infrastructure, or barter.
		// Citizen-soldiers remain on wood/building/military duty.
		return;
	...
	applyFoodRecoveryRebalance(gameState, feedback)
	{
		...
		for (const ent of gameState.getOwnUnits().values())
		{
			if (!ent || !entityPosition(ent) || !this.isExpertEconomyEntity(ent) ||
				...
				continue;
			const assignedJob = ent.getMetadata(PlayerID, JOB_METADATA);
			const supplyId = Number(ent.getMetadata(PlayerID, SUPPLY_ID));
			const supply = Number.isFinite(supplyId) ? gameState.getEntityById(supplyId) : undefined;
			const actualResource = this.resourceGenericForSupply(supply) || ent.getMetadata(PlayerID, "gather-type");
			// IT15.0: move the worker who is ACTUALLY chopping wood, not only one whose
			// stale permanent metadata happens to say "wood".
			if (assignedJob !== "wood" && actualResource !== "wood")
				continue;
		...
		for (const item of candidates)
		{
			...
			if (item.productive && (!productiveWindowOpen || productiveMoved >= productiveBatch))
				continue;
			const ent = item.ent;
			const carrying = ent.resourceCarrying ? (ent.resourceCarrying() || []) : [];
	...
	workerActualResource(gameState, ent)
	{
		...
		const supply = Number.isFinite(supplyId) ? gameState.getEntityById(supplyId) : undefined;
		const state = ent.unitAIState ? String(ent.unitAIState() || "") : "";
		if (supply && ["food", "wood", "stone", "metal"].includes(this.resourceGenericForSupply(supply)) &&
		    (hasLiveGatherOrder(ent, supply.id()) || state.includes("GATHER")))
			return this.resourceGenericForSupply(supply);
		const carrying = ent.resourceCarrying ? (ent.resourceCarrying() || []) : [];
		const carriedType = carrying.find(item => item && ["food", "wood", "stone", "metal"].includes(item.type) && Number(item.amount) > 0);
		if (carriedType && (state.includes("RETURNRESOURCE") || state.includes("RETURNINGRESOURCE")))
	...
	redirectWorkerToForecastResource(gameState, ent, target)
	{
		...
		// IT14.92: if this is already a temporary fallback worker, preserve its long-
		// term ownership. In particular, a future farmer physically mining surplus
		// stone should be sent to needed wood WITHOUT becoming a permanent lumberjack.
		// The farm-crew system can then reclaim the same civilian when capacity opens.
		const metadataResource = jobResourceType(current);
		const preserveOwnership = ent.getMetadata(PlayerID, EXPERT_FUTURE_FARMER) === true ||
			current === "food_owned" || current === "farm" || (metadataResource && metadataResource !== actual);
		const carrying = ent.resourceCarrying ? (ent.resourceCarrying() || []) : [];
		...
		if (preserveOwnership)
		{
			...
			{
				returnResources(gameState, ent);
				ent.setMetadata(PlayerID, EXPERT_FALLBACK_LEASE_RESOURCE, target);
				ent.setMetadata(PlayerID, EXPERT_FALLBACK_LEASE_UNTIL, (Number(gameState.ai.elapsedTime) || 0) + mergePolicy().temporaryFallbackLeaseSeconds);
				ent.setMetadata(PlayerID, SUPPLY_ID, undefined);
				ent.setMetadata(PlayerID, "gather-type", undefined);
				return true;
			}
			const access = getLandAccess(gameState, ent);
			return this.assignSafeFallback(gameState, ent, access, [target]);
		}

		if (current !== desired)
			return this.setDesiredJob(gameState, ent, desired);
		ent.setMetadata(PlayerID, SUPPLY_ID, undefined);
		ent.setMetadata(PlayerID, "gather-type", undefined);
		ent.setMetadata(PlayerID, PENDING_JOB_METADATA, undefined);
		ent.setMetadata(PlayerID, EXPERT_JOB_LEASE_UNTIL, (Number(gameState.ai.elapsedTime) || 0) + mergePolicy().resourceJobLeaseSeconds);
		ent.setMetadata(PlayerID, EXPERT_JOB_LEASE_RESOURCE, target);
		if (ent.stopMoving) ent.stopMoving();
		if (!hasClass(ent, "Cavalry")) ent.setMetadata(PlayerID, "subrole", Worker.SUBROLE_IDLE);
		return true;
	}

	forecastFallbackPreference(gameState, ent, job, preferred)
	{
		const base = [...new Set((preferred || []).filter(type => ["food", "wood", "stone", "metal"].includes(type)))];
		if (!this.resourceForecast || !base.length)
			return base;
		const policy = mergePolicy();
		const now = Number(gameState.ai.elapsedTime) || 0;
		const permanent = jobResourceType(job);
		// Protect the scripted opening from the old "wood order missed -> everybody mines
		// stone" fallback. Before the adaptive economy starts, only food/wood overflow is
		// sensible unless a worker was explicitly assigned to a finite resource.
		if (now < (Number(policy.resourceBalanceStartTime) || 165))
		{
			if (permanent === "stone" || permanent === "metal")
				return [permanent, "wood"].filter(type => base.includes(type));
			if (permanent === "food" || ent.getMetadata(PlayerID, EXPERT_FUTURE_FARMER) === true)
				return ["food", "wood"].filter(type => base.includes(type));
			return base.includes("wood") ? ["wood"] : base.slice(0, 1);
		}
		const maxCoverage = Math.max(1, Number(policy.resourceForecastFallbackMaximumCoverage) || 1.6);
		const useful = base.map(type => this.resourceForecast.resources[type]).filter(Boolean).filter(resource =>
			resource.status === "critical" || resource.status === "short" ||
			(resource.status !== "surplus" && resource.coverageRatio < maxCoverage));
		useful.sort((a, b) =>
			Number(b.status === "critical") - Number(a.status === "critical") ||
			Number(b.status === "short") - Number(a.status === "short") ||
			b.needScore - a.needScore || a.coverageRatio - b.coverageRatio);
		return useful.map(resource => resource.type);
	}
	...
	rebalanceExistingWorkers(gameState, openingEnd, balance)
	{
		...
		if (!balance || !balance.active || !balance.strong || now < policy.resourceBalanceStartTime)
			return;
		const critical = !!balance.forecastCritical;
		const extreme = !!balance.extreme || Number(balance.ratio || 1) >= policy.resourceBalanceExtremeRatio;
		const cooldown = critical ? (Number(policy.resourceForecastCriticalReassignCooldownSeconds) || 3) :
			balance.forecast ? (Number(policy.resourceForecastReassignCooldownSeconds) || 6) :
			extreme ? policy.resourceBalanceExtremeCooldownSeconds : policy.resourceBalanceReassignCooldownSeconds;
		if (now - this.lastResourceRebalanceTime < cooldown)
			return;

		const donorOrder = [];
		if (balance.surplus) donorOrder.push(balance.surplus);
		if (this.resourceForecast && this.resourceForecast.rankedSurplus)
			for (const resource of this.resourceForecast.rankedSurplus)
				if (resource && resource.type !== balance.target && !donorOrder.includes(resource.type) &&
				    (resource.status === "surplus" || resource.floatAmount >= 300 || resource.coverageRatio >= 1.5))
					donorOrder.push(resource.type);
		const donorRank = new Map(donorOrder.map((type, index) => [type, index]));
		const rushFoodCivilianDoctrine = this.strategyDoctrine &&
		...
		for (const ent of gameState.getOwnUnits().values())
		{
			...
			// IT15.1: generic forecast balancing may move citizen-soldiers, but it may not
			// dismantle the rush's dedicated-civilian food backbone after syncJobs built it.
			if (civilian && rushFoodCivilianDoctrine)
				continue;
			...
			if (civilian && balance.target === "wood" && !extreme && !critical)
			{
				const actualNow = this.workerActualResource(gameState, ent);
				const metadataNow = jobResourceType(ent.getMetadata(PlayerID, JOB_METADATA));
				if (!actualNow || actualNow === metadataNow)
					continue;
			}
			const lockedFieldId = Number(ent.getMetadata(PlayerID, FARM_LOCK));
			...
			const current = ent.getMetadata(PlayerID, JOB_METADATA);
			const metadataResource = jobResourceType(current);
			const actualResource = this.workerActualResource(gameState, ent);
			const donorResource = actualResource || metadataResource;
			if (!donorRank.has(donorResource))
				continue;

			if (!soldier && !civilian)
				continue;
			const ordinal = Number(ent.getMetadata(PlayerID, CIVILIAN_ORDINAL));
			const temporaryMismatch = !!(actualResource && metadataResource && actualResource !== metadataResource);
			// IT15.3 worker inertia: do not turn around a worker who is already travelling
			// or returning a load. The previous critical-forecast path bypassed this and
			// repeatedly converted useful labor into cross-map walking.
			const travelling = state.includes("GATHER.APPROACHING") || state.includes("GATHER.WALKING") ||
				state.includes("RETURNINGRESOURCE") || state.includes("RETURNRESOURCE");
			if (travelling && !temporaryMismatch)
				continue;
			const productive = !!(actualResource && actualResource === donorResource && state.includes("GATHER.GATHERING"));
			const idle = !!(ent.isIdle && ent.isIdle());
			const lastStrategicMove = Number(ent.getMetadata(PlayerID, EXPERT_STRATEGIC_MOVE_AT));
			if (Number.isFinite(lastStrategicMove) && now - lastStrategicMove <
			    (Number(policy.resourceProductiveReassignCooldownSeconds) || 12))
				continue;
			// Opening ownership remains protected, but an opening food/wood civilian who is
			// TEMPORARILY mining a surplus is not really a mineral worker. Reclaim that
			// accidental fallback immediately instead of letting metadata hide it.
			if (civilian && (!Number.isFinite(ordinal) || ordinal <= openingEnd) && !temporaryMismatch)
				continue;
			candidates.push({ ent, soldier, ordinal: Number.isFinite(ordinal) ? ordinal : 0,
				actualResource, metadataResource, donorResource, temporaryMismatch, productive, idle });
		}

		// IT15.3: cheapest moves first: accidental fallback mismatch, truly idle workers,
		// then flexible non-gathering soldiers. Productive gatherers are LAST and capped
		// separately, so a forecast shortage cannot make 20-50 workers march together.
		candidates.sort((a, b) => Number(b.temporaryMismatch) - Number(a.temporaryMismatch) ||
			Number(b.idle) - Number(a.idle) || Number(a.productive) - Number(b.productive) ||
			(donorRank.get(a.donorResource) || 0) - (donorRank.get(b.donorResource) || 0) ||
			Number(b.soldier) - Number(a.soldier) || b.ordinal - a.ordinal || b.ent.id() - a.ent.id());
		const baseTarget = critical ? Math.min(Number(policy.resourceForecastCriticalReassignBatch) || 10,
			Number(policy.resourceCriticalTotalReassignBatch) || 5) :
			balance.forecast ? (Number(policy.resourceForecastReassignBatch) || 4) :
			extreme ? policy.resourceBalanceExtremeBatch : policy.resourceBalanceReassignBatch;
		...
		for (const item of candidates)
		{
			...
			const carrying = ent.resourceCarrying ? (ent.resourceCarrying() || []) : [];
			const carried = carrying.reduce((sum, item2) => sum + Math.max(0, Number(item2 && item2.amount) || 0), 0);
			if (!this.redirectWorkerToForecastResource(gameState, ent, balance.target))
				continue;
			ent.setMetadata(PlayerID, EXPERT_STRATEGIC_MOVE_AT, now);
			++moved;
			if (item.productive)
				++productiveMoved;
			aiWarn((balance.forecast ? "[EXPERT-FORECAST]" : "[EXPERT-BALANCE]") + " peel worker=" + ent.id() + " " + item.donorResource + "->" + balance.target +
				(item.temporaryMismatch ? " actual-over-metadata=1" : "") +
				" ratio=" + Number(balance.ratio || 1).toFixed(2) + (critical ? " critical=1" : "") +
				(carried > 0 ? " deposit-first=" + Math.round(carried) : ""));
		}
		if (moved)
			this.lastResourceRebalanceTime = now;
		if (productiveMoved)
			this.lastProductiveResourceRebalanceTime = now;
	}
	...
	resourceJobChangeUrgent(gameState, current, desired)
	{
		...
		const bank = gameState.getResources();
		// IT14.91: forecast-critical means the next planned purchases cannot be funded
		// by bank + accessible stock + income. That is a genuine cross-resource emergency
		// for any resource, not only the old food/wood special cases.
		if (target && this.forecastResourceCritical(target))
			return true;
		const forecastMove = this.resourceForecast ? this.forecastDirective(["food", "wood", "stone", "metal"]) : undefined;
		if (target && forecastMove && forecastMove.active && forecastMove.strong && forecastMove.target === target &&
		    jobResourceType(current) === forecastMove.surplus)
			return true;
		if (target === "food")
	...
	setDesiredJob(gameState, ent, desired, options = {})
	{
		if (!ent || !ent.getMetadata || !ent.setMetadata)
			return false;
		const current = ent.getMetadata(PlayerID, JOB_METADATA);
		...
		const urgent = forced || (cross && this.resourceJobChangeUrgent(gameState, current, desired));
		...
		if (cross)
		{
			...
			ent.setMetadata(PlayerID, EXPERT_JOB_LEASE_UNTIL, now + mergePolicy().resourceJobLeaseSeconds);
			ent.setMetadata(PlayerID, EXPERT_JOB_LEASE_RESOURCE, jobResourceType(desired));
			// A forced doctrine transfer must also cancel the old engine gather order;
			// changing metadata alone does not make a lumberjack stop chopping.
			if (forced && ent.stopMoving)
				ent.stopMoving();
		}
		ent.setMetadata(PlayerID, JOB_METADATA, desired);
		ent.setMetadata(PlayerID, PENDING_JOB_METADATA, undefined);
	...
	commitCompletedStorehouseBuilders(gameState, taskId, storehouseId)
	{
		...
		const openingWoodStorehouse = this.builtByClass(gameState, "Storehouse").length <= 1;
		const generic = intent.resourceGeneric || (this.pendingWoodSelectionByTask[taskId] ? "wood" :
			openingWoodStorehouse ? "wood" : undefined);
		for (const ent of team)
			this.releaseConstructionWorker(ent, taskId);
		if (!store || !entityPosition(store))
			return;
		// IT14.92: a Storehouse is not automatically a lumber camp. 14.91 converted
		// builders of mixed/mineral Storehouses into woodcutters, which both corrupted
		// worker allocation and made building placement change economic strategy.
		if (generic && generic !== "wood")
		{
			aiWarn("[EXPERT-SERVICE] completed storehouse preserves builders resource=" + generic +
				" site=" + store.id() + " workers=" + team.length);
			return;
		}
		if (!generic && !this.pendingWoodSelectionByTask[taskId])
			return;
		let committed = 0;
		for (const ent of team)
		{
	...
	applyOpeningBuildingPassThrough(gameState, cc)
	{
		...
		for (const ent of gameState.getOwnUnits().values())
		{
			if (!ent || !ent.getMetadata || Number(ent.getMetadata(PlayerID, "expertOpeningTeleportHolder")) !== cc.id())
				continue;
			...
			if (ent.garrisonHolderID && ent.garrisonHolderID() === cc.id() && cc.unload)
			{
				...
				if (!Number.isFinite(unloadAt) && now - rallyAt >= 1)
				{
					// Preserve the directional rally point through the actual unload. Clearing
					// it in the rally-setting turn let the engine choose its default exit side.
					cc.unload(ent.id());
		...
		if (!ccPos || !cc.canGarrisonInside || !cc.canGarrisonInside())
			return;
		const obstruction = cc.obstructionRadius ? cc.obstructionRadius() : undefined;
		...
		for (const ent of gameState.getOwnUnits().values())
		{
			...
			if (!finish || SquareVectorDistance(start, ccPos) > (holderRadius + 14) * (holderRadius + 14) ||
			    ...
			    pointSegmentDistanceSquared(ccPos, start, finish) > (holderRadius + 2) * (holderRadius + 2))
				continue;
			const ax = start[0] - ccPos[0], az = start[1] - ccPos[1];
			const bx = finish[0] - ccPos[0], bz = finish[1] - ccPos[1];
	...
	commitCompletedNaturalFarmsteadBuilders(gameState, taskId, cluster)
	{
		...
		for (const ent of team)
		{
			...
			// A worker recycled from the exhausted first patch can still carry its
			// permanent-field preference. The new live berry lock must win until
			// these supplies are exhausted, even beside an existing Farmstead.
			ent.setMetadata(PlayerID, FOOD_HOME_PERMANENT, undefined);
			ent.setMetadata(PlayerID, SUPPLY_ID, undefined);
	...
	releaseFutureFieldReservations(gameState, taskId)
	{
		...
		for (const ent of gameState.getOwnUnits().values())
		{
			if (!ent || !ent.getMetadata || !ent.setMetadata)
				continue;
			const reserved = ent.getMetadata(PlayerID, EXPERT_FUTURE_FARM_TASK);
			if (reserved !== undefined && String(reserved) === String(taskId))
				ent.setMetadata(PlayerID, EXPERT_FUTURE_FARM_TASK, undefined);
		}
	}
	...
	lockCompletedFieldBuilders(gameState, taskId, fieldId)
	{
		...
		this.releaseFutureFieldReservations(gameState, taskId);
		...
		for (const ent of team)
		{
			...
			ent.setMetadata(PlayerID, EXPERT_FUTURE_FARMER, undefined);
			ent.setMetadata(PlayerID, JOB_METADATA, "farm");
			ent.setMetadata(PlayerID, PENDING_JOB_METADATA, undefined);
	...
	maintainFieldConstructionCrew(gameState, taskId, observed)
	{
		...
		if (!foundation || !entityPosition(foundation))
			return;
		const intent = this.activeTaskBuildIntent[taskId] || {};
		const desired = Math.max(1, Number(intent.builderCount) || Number(desiredBuilders("field")) || 4);
		const existing = this.constructionWorkers(gameState, taskId);
		const action = {
			"type": "MAINTAIN_CONSTRUCTION",
			"kind": "field",
			"builderPool": Array.isArray(intent.builderPool) ? [...intent.builderPool] : ["farm", "food_owned", "food"],
			"requiredBuilderIds": Array.isArray(intent.requiredBuilderIds) ? [...intent.requiredBuilderIds] : undefined,
			"builderCount": desired,
			"builderJobPriority": intent.builderJobPriority ? { ...intent.builderJobPriority } : { "farm": 5, "food_owned": 4, "food": 3 }
		};
		let team = [];
		try
		{
	...
	woodTreesAt(gameState, position, accessIndex, radius = 30)
	{
		return collectWoodTrees(gameState, {
			...
			"playerId": PlayerID,
			"radius": radius,
			// IT15.1: serviced wood must still lie inside our territory.
			"allowNeutralWood": false
		});
	}
	...
	// IT14.89: non-opening Storehouses are evaluated against the resource mass they
	// actually service, not just the one tree/mine that happened to be a worker target.
	resourcePlacementPoints(gameState, ids)
	{
		const points = [];
		for (const id of ids || [])
		{
			...
			if (!ent || !entityPosition(ent))
				continue;
			let weight = ent.resourceSupplyAmount ? Math.max(1, Number(ent.resourceSupplyAmount()) || 0) : 1;
			points.push({ "position": [...ent.position()], weight, "generic": this.resourceGenericForSupply(ent) });
		}
		return points.sort((a, b) => b.weight - a.weight).slice(0, 48);
	}

	weightedResourceCenter(points)
	{
		let total = 0, x = 0, z = 0;
		for (const point of points || [])
		{
			if (!point || !Array.isArray(point.position))
				continue;
			const weight = Math.max(0.001, Number(point.weight) || 1);
			total += weight;
			x += point.position[0] * weight;
			z += point.position[1] * weight;
		}
		return total > 0 ? [x / total, z / total] : undefined;
	}

	storehouseAverageDistanceGain(gameState, generic, points, candidate)
	{
		if (!Array.isArray(candidate) || !(points && points.length))
			return 0;
		let totalWeight = 0, oldDistance = 0, newDistance = 0;
		for (const point of points)
		{
			if (!point || !Array.isArray(point.position))
				continue;
			const pointGeneric = point.generic || generic;
			const dropsites = this.resourceDropsites(gameState, pointGeneric).filter(ent => ent && entityPosition(ent));
			if (!dropsites.length)
				continue;
			const weight = Math.max(0.001, Number(point.weight) || 1);
			let old = Infinity;
			for (const dropsite of dropsites)
				old = Math.min(old, Math.sqrt(SquareVectorDistance(point.position, dropsite.position())));
			const proposed = Math.sqrt(SquareVectorDistance(point.position, candidate));
			totalWeight += weight;
			oldDistance += old * weight;
			newDistance += Math.min(old, proposed) * weight;
		}
		return totalWeight > 0 ? oldDistance / totalWeight - newDistance / totalWeight : Infinity;
	}

	liveWoodCuttingFront(gameState, accessIndex, primaryPosition, primaryEntityId)
	{
		const policy = mergePolicy();
		const targetRecords = [];
		...
		for (const worker of gameState.getOwnUnits().values())
		{
			if (!worker || !worker.getMetadata || !this.isExpertEconomyEntity(worker) ||
			    worker.getMetadata(PlayerID, "PartOfArmy") || worker.getMetadata(PlayerID, TASK_KEY) !== undefined)
				continue;
			const supplyId = Number(worker.getMetadata(PlayerID, SUPPLY_ID));
			const supply = Number.isFinite(supplyId) ? gameState.getEntityById(supplyId) : undefined;
			if (!supply || !entityPosition(supply) || this.resourceGenericForSupply(supply) !== "wood" ||
			    !supply.resourceSupplyAmount || supply.resourceSupplyAmount() <= 0 || getLandAccess(gameState, supply) !== accessIndex)
				continue;
			const assigned = Number(worker.getMetadata(PlayerID, WORKSITE_ID));
			if (Number.isFinite(Number(primaryEntityId)) && Number.isFinite(assigned) && assigned !== Number(primaryEntityId))
				continue;
			if (!Number.isFinite(assigned) && Array.isArray(primaryPosition) &&
			    SquareVectorDistance(primaryPosition, supply.position()) > Math.pow(Number(policy.woodCuttingFrontAssociationDistance) || 62, 2))
				continue;
			++workerCount;
			if (!targetIds.has(supply.id()))
			{
				targetIds.add(supply.id());
				targetRecords.push({ id: supply.id(), position: [...supply.position()], remaining: Math.max(0, Number(supply.resourceSupplyAmount()) || 0) });
			}
		}
		if (workerCount < Math.max(1, Number(policy.woodCuttingFrontMinimumWorkers) || 3) || !targetRecords.length)
			return undefined;
		const targetCenter = this.weightedResourceCenter(targetRecords.map(tree => ({ position: tree.position, weight: Math.max(1, tree.remaining) })));
		if (!targetCenter)
			return undefined;
		const nearby = collectInitialWoodCandidates(gameState, {
			"getLandAccess": getLandAccess, "isSupplyFull": isSupplyFull,
			"territoryMap": this.HQ.territoryMap, "anchorPosition": targetCenter,
			"accessIndex": accessIndex, "playerId": PlayerID,
			"searchRadius": Math.max(40, Number(policy.woodCuttingFrontSearchRadius) || 82),
			"allowNeutralWood": false
		});
		const fringe2 = Math.pow(Math.max(Number(policy.woodCuttingFrontFringeDistance) || 38,
			Number(policy.woodClusterLinkDistance) || 22), 2);
		const practical = nearby.filter(tree => targetRecords.some(target => SquareVectorDistance(tree.position, target.position) <= fringe2));
	...
	primaryWoodDistrictHealth(gameState, accessIndex)
	{
		const policy = mergePolicy();
		const current = this.getPrimaryWoodPosition(gameState);
		if (!current)
			return { healthy: false, remaining: 0, activeWorkers: 0, stores: 0, treeIds: [], center: undefined };
		const nearby = collectInitialWoodCandidates(gameState, {
			"getLandAccess": getLandAccess, "isSupplyFull": isSupplyFull,
		...
		const connected = this.connectedWoodCluster(nearby, current, Number(policy.woodClusterLinkDistance) || 22);
		const fringe2 = Math.pow(Math.max(Number(policy.woodPracticalDistrictFringeDistance) || 34,
			Number(policy.woodClusterLinkDistance) || 22), 2);
		const connectedIds = new Set(connected.map(tree => tree.id));
		let practical = connected.length ? nearby.filter(tree => connectedIds.has(tree.id) ||
			connected.some(base => SquareVectorDistance(tree.position, base.position) <= fringe2)) : connected;
		// IT14.92: the economic district follows the actual cutting front. Trees being
		// worked by lumberjacks committed to this Storehouse remain part of the district
		// even after the forest edge has receded beyond the old Storehouse-centred ring.
		const liveFront = this.liveWoodCuttingFront(gameState, accessIndex, current,
			this.primaryWoodWorksite && this.primaryWoodWorksite.entityId);
		if (liveFront && liveFront.trees && liveFront.trees.length)
		{
			const merged = new Map(practical.map(tree => [Number(tree.id), tree]));
			for (const tree of liveFront.trees) merged.set(Number(tree.id), tree);
			practical = [...merged.values()];
		}
		const ids = new Set(practical.map(tree => Number(tree.id)));
		let activeWorkers = 0;
		for (const worker of gameState.getOwnUnits().values())
		{
			if (!worker || !worker.getMetadata || !this.isExpertEconomyEntity(worker) || worker.getMetadata(PlayerID, "PartOfArmy"))
				continue;
			const supplyId = Number(worker.getMetadata(PlayerID, SUPPLY_ID));
			if (!ids.has(supplyId))
				continue;
			const state = worker.unitAIState ? String(worker.unitAIState() || "") : "";
			if (!worker.isIdle || !worker.isIdle() || state.includes("GATHER"))
				++activeWorkers;
		}
		const remaining = this.woodAmount(practical);
		const stores = this.storehousesServingWoodCluster(gameState, practical);
		const bank = Math.max(0, Number(gameState.getResources().wood) || 0);
		const immediateCrisis = bank <= (Number(policy.woodCrisisImmediateBank) || 250) &&
			activeWorkers <= (Number(policy.woodCrisisImmediateActiveWorkers) || 2);
		const healthy = remaining >= (Number(policy.woodHealthyDistrictMinimumRemaining) || 900) &&
			activeWorkers >= (Number(policy.woodHealthyDistrictMinimumActiveWorkers) || 4) && stores >= 1 &&
			!this.woodIncomeStalled && !this.phaseWoodCrisis && !immediateCrisis;
		return { healthy, remaining, activeWorkers, stores, treeIds: [...ids], center: this.weightedResourceCenter(practical.map(tree => ({ position: tree.position, weight: tree.remaining }))) };
	}
	...
	collectWoodsite(gameState, cc, accessIndex)
	{
		...
		{
			// IT14.92: first ask where the committed lumberjacks are actually cutting.
			// A receding forest edge is not an exhausted district merely because it moved
			// outside the original Storehouse radius. Keep the crew here and let the
			// Storehouse planner advance the dropsite along this cutting front.
			const liveFront = this.liveWoodCuttingFront(gameState, accessIndex, pos, entityId);
			if (liveFront && liveFront.workers >= (Number(policy.woodCuttingFrontMinimumWorkers) || 3) &&
			    liveFront.remaining >= (Number(policy.woodCuttingFrontAdvanceMinimumRemaining) || 450))
			{
				trees = liveFront.trees;
				metrics = summarizeWoodTrees(trees);
			}
			else
			{
	...
	scarcityExpansionContext(gameState, frame, woodsite)
	{
		...
		const longHaul = Math.max(0, Number(woodsite && woodsite.averageDropDistance) || 0) >=
		...
		const forecastNeed = this.resourceForecast && this.resourceForecast.rankedNeeds ?
			this.resourceForecast.rankedNeeds.find(item => item && item.status === "critical" &&
				item.ownAccessible + item.neutralAccessible < item.mediumDemand) : undefined;
		const forecastCritical = !!forecastNeed;
		const forecastWood = this.resourceForecast && this.resourceForecast.resources && this.resourceForecast.resources.wood;
		const forecastWoodComfortable = !!(forecastWood && (forecastWood.status === "balanced" || forecastWood.status === "surplus"));
		// Local forest exhaustion is not strategic scarcity if 7k wood already covers the
		// projected economy. Conversely, a forecast-critical resource may trigger expansion
	...
	cleruchyFrontierCandidate(gameState, cc, accessIndex, scarcity = false, primaryResource = undefined)
	{
		...
		if (primaryResource && weights[primaryResource])
	...
	applyRecoveryMarketInfrastructure(gameState, frame)
	{
		...
		const forecastNeed = this.resourceForecast && this.resourceForecast.rankedNeeds && this.resourceForecast.rankedNeeds[0];
		const forecastDonor = this.resourceForecast && this.resourceForecast.rankedSurplus && this.resourceForecast.rankedSurplus[0];
		const forecastBarterNeed = !!(forecastNeed && forecastDonor && forecastNeed.type !== forecastDonor.type &&
			(forecastNeed.status === "critical" || forecastNeed.status === "short") &&
			forecastDonor.floatAmount >= (Number(policy.resourceForecastBarterMinimumDonorFloat) || 900));
		const woodForecast = this.resourceForecast && this.resourceForecast.resources && this.resourceForecast.resources.wood;
		const nonWoodBank = Math.max(0, Number(bank.food) || 0) + Math.max(0, Number(bank.stone) || 0) + Math.max(0, Number(bank.metal) || 0);
		const pivotMarketNeed = this.strategyWoodPivot && woodForecast && (woodForecast.status === "critical" || woodForecast.status === "short") &&
			nonWoodBank >= (Number(policy.strategyWoodPivotMarketMinimumDonorBank) || 650);
		if ((!foodLow || surplus < (Number(policy.expertRecoveryMarketSurplusTrigger) || 2200)) && !forecastBarterNeed && !pivotMarketNeed)
			return frame;
		const actions = [...(frame.actions || [])];
	...
	applyForecastBarter(gameState)
	{
		if (!gameState.currentPhase || gameState.currentPhase() < 2 || !this.resourceForecast)
			return false;
		const policy = mergePolicy();
		const now = Number(gameState.ai.elapsedTime) || 0;
		...
		if (amount < 100)
			return false;
		try { barterer.barter(target.type, donor.type, amount); }
		catch (e) { return false; }
		this.lastForecastBarterAt = now;
		aiWarn("[EXPERT-FORECAST] barter sell=" + donor.type + " buy=" + target.type + " amount=" + Math.round(amount) +
			" donorFloat=" + Math.round(donor.floatAmount) + " target=" + target.status + ":" + target.coverageRatio.toFixed(2));
		return true;
	}
	...
	applyAthenianFrontierCleruchy(gameState, frame, cc, accessIndex, woodsite)
	{
		...
		if (scarcity.primaryResource === "food" && (frame.actions || []).some(action => action && action.kind === "farmstead" &&
		    (action.role === "natural_frontier" || action.role === "natural_expansion")))
			return frame;
		...
		const forecastP1 = p1Scarcity && scarcity.forecastCritical;
		const minTime = forecastP1 ? (Number(policy.athensCleruchyForecastCriticalP1MinimumTime) || 210) : p1Scarcity ? policy.athensCleruchyScarcityP1MinimumTime :
			scarcity.active ? policy.athensCleruchyScarcityMinimumTime : policy.athensCleruchyMinimumTime;
		const minPop = forecastP1 ? (Number(policy.athensCleruchyForecastCriticalP1MinimumPopulation) || 40) : p1Scarcity ? policy.athensCleruchyScarcityP1MinimumPopulation :
	...
	applyScarcityBaseExpansion(gameState, queues, frame, cc, accessIndex, woodsite)
	{
		...
		const scarcity = this.scarcityExpansionContext(gameState, frame, woodsite);
		const finishing = this.finishingState(gameState);
		const minimumPopulation = this.strategyWoodPivot && scarcity.primaryResource === "wood" ?
			(Number(mergePolicy().strategyWoodPivotBaseExpansionMinimumPopulation) || 55) : 75;
		if (!scarcity.active || gameState.getPopulation() < minimumPopulation ||
	...
	{
		// IT15.1 sovereignty: neutral food may motivate a future territorial expansion,
		// but it is never an economic gather/Farmstead target before ownership changes.
		return undefined;
	}


	// IT15.1 compatibility stub. Neutral food does not preempt Fields; territorial
	// expansion must claim the resource first.
	applyForecastNaturalFoodExpansion(gameState, frame, cc, accessIndex, foodNetwork)
	{
		return frame;
	}


	// IT15.3: restored after the IT15.1 sovereignty cleanup accidentally removed the
	// helpers while leaving applyForecastStorehouseInfrastructure() calling them.
	// Current-economy service is OWN TERRITORY ONLY; neutral stock remains expansion
	// intelligence and can never become an immediate Storehouse/gather target.
	resourceWorkersBySupply(gameState)
	{
		...
		for (const worker of gameState.getOwnUnits().values())
		{
			if (!worker || !worker.getMetadata || !this.isExpertEconomyEntity(worker) || worker.getMetadata(PlayerID, "PartOfArmy"))
				continue;
			const id = Number(worker.getMetadata(PlayerID, SUPPLY_ID));
			if (Number.isFinite(id))
	...
	forecastStorehouseCandidate(gameState, cc, accessIndex)
	{
		if (!cc || !entityPosition(cc) || !gameState.getResourceSupplies)
			return undefined;
		const policy = mergePolicy();
		const ccPos = cc.position();
		const maxRadius = Math.max(50, Number(policy.resourceForecastStorehouseSearchRadius) || 115);
		const maxRadius2 = maxRadius * maxRadius;
		const clusterRadius = Math.max(20, Number(policy.resourceForecastStorehouseRadius) || 42);
		const clusterRadius2 = clusterRadius * clusterRadius;
		const workers = this.resourceWorkersBySupply(gameState);
		const weights = { wood: Number(policy.resourceForecastStorehouseWoodWeight) || 1,
			stone: Number(policy.resourceForecastStorehouseStoneWeight) || 0.75,
			metal: Number(policy.resourceForecastStorehouseMetalWeight) || 0.85 };
		const workerBonus = Math.max(0, Number(policy.resourceForecastStorehouseWorkerBonus) || 120);
		const records = [];
		const existingStores = this.builtByClass(gameState, "Storehouse").filter(ent => entityPosition(ent));
		for (const generic of ["wood", "stone", "metal"])
			for (const supply of gameState.getResourceSupplies(generic).values())
			{
				const pos = entityPosition(supply);
				if (!pos || !supply.resourceSupplyAmount || getLandAccess(gameState, supply) !== accessIndex)
					continue;
				// IT15.1/15.3 sovereignty contract: proactive service can only consider
				// resources we already own.
				if (this.HQ.territoryMap.getOwner(pos) !== PlayerID)
					continue;
				let footprintDistance = SquareVectorDistance(pos, ccPos);
				for (const store of existingStores)
					footprintDistance = Math.min(footprintDistance, SquareVectorDistance(pos, store.position()));
				if (footprintDistance > maxRadius2)
					continue;
				const amount = Math.min(Number(supply.resourceSupplyAmount()) || 0,
					Math.max(100, Number(policy.resourceForecastSupplyCapPerEntity) || 5000));
				if (amount <= 0)
					continue;
				records.push({ id: supply.id(), generic, position: [...pos], amount,
					workers: workers.get(supply.id()) || 0, owner: PlayerID });
			}
		if (!records.length)
			return undefined;
		const seeds = records.slice().sort((a, b) =>
			Number(b.generic !== "wood") - Number(a.generic !== "wood") || b.workers - a.workers || b.amount - a.amount).slice(0, 96);
		let best;
		for (const seed of seeds)
		{
			const cluster = records.filter(record => SquareVectorDistance(seed.position, record.position) <= clusterRadius2);
			if (!cluster.length)
				continue;
			let value = 0, workerCount = 0;
			const types = new Set();
			for (const record of cluster)
			{
				value += record.amount * weights[record.generic] + record.workers * workerBonus;
				workerCount += record.workers;
				types.add(record.generic);
			}
			const points = cluster.map(record => ({ position: record.position,
				weight: Math.max(1, record.amount * weights[record.generic] + record.workers * workerBonus), generic: record.generic }));
			const center = this.weightedResourceCenter(points);
			if (!center)
				continue;
			const critical = cluster.some(record => this.forecastResourceCritical(record.generic));
			// Dedicated wood rollover owns wood-only camps. This routine is for mixed or
			// mineral service and must not create duplicate lumber camps.
			if (types.size === 1 && cluster[0].generic === "wood")
				continue;
			if (workerCount < 1 && !critical)
				continue;
			const minValue = critical ? Number(policy.resourceForecastStorehouseCriticalMinimumValue) || 350 :
				Number(policy.resourceForecastStorehouseMinimumValue) || 700;
			if (value < minValue)
				continue;
			const gain = this.storehouseAverageDistanceGain(gameState, "mixed", points, center);
			const minGain = critical ? Number(policy.resourceForecastStorehouseCriticalMinimumGain) || 2 :
				Number(policy.resourceForecastStorehouseMinimumGain) || 4;
			if (Number.isFinite(gain) && gain < minGain)
				continue;
			const score = value + workerCount * workerBonus + (Number.isFinite(gain) ? gain * 150 : 3000) + types.size * 180;
			if (!best || score > best.score)
				best = { center, records: cluster, value, workerCount, resourceTypes: types.size, gain, critical, score };
		}
		return best;
	}

	applyForecastStorehouseInfrastructure(gameState, frame, cc, accessIndex)
	{
		const policy = mergePolicy();
		const now = Number(gameState.ai.elapsedTime) || 0;
		if (now < (Number(policy.resourceForecastStorehouseStartTime) || 105) ||
		    now - this.lastForecastStorehouseAt < (Number(policy.resourceForecastStorehouseCooldownSeconds) || 24) ||
		    this.builtByClass(gameState, "Storehouse").length < 1 || this.foundationsByClass(gameState, "Storehouse").length ||
		    this.activeTaskByKind.storehouse)
			return frame;
		if ((frame.actions || []).some(action => action && action.kind === "storehouse" && (action.type === "BUILD" || action.type === "RESERVE")))
			return frame;
		// Scanning tree clusters is intentionally periodic even when no candidate wins.
		this.lastForecastStorehouseAt = now;
		const desiredFields = frame && frame.economy && frame.economy.derived ?
			Math.max(0, Number(frame.economy.derived.desiredFields) || 0) : 0;
		const fieldPipeline = this.builtByClass(gameState, "Field").length + this.foundationsByClass(gameState, "Field").length;
		const foodForecast = this.resourceForecast && this.resourceForecast.resources && this.resourceForecast.resources.food;
		const naturalRemaining = frame && frame.state && frame.state.food ?
			Math.max(0, Number(frame.state.food.totalNaturalRemaining) || 0) : 0;
		if (fieldPipeline < desiredFields && (naturalRemaining <= 0 ||
		    foodForecast && (foodForecast.status === "critical" || foodForecast.status === "short")))
		{
			// IT15.1: do not spend the Field wood on speculative mixed/mineral service
			// while permanent food capacity is behind target. Dedicated wood rollover is
			// handled separately and may still build the Storehouse needed to reach owned wood.
			return frame;
		}
		const candidate = this.forecastStorehouseCandidate(gameState, cc, accessIndex);
		if (!candidate)
			return frame;
		const cost = Number(policy.costs && policy.costs.storehouse && policy.costs.storehouse.wood) || 100;
		if ((Number(gameState.getResources().wood) || 0) < cost && !candidate.critical)
			return frame;
		const ids = candidate.records.map(record => record.id);
		const actions = [...(frame.actions || []), {
			type: "BUILD", kind: "storehouse", role: "resource_service", priority: candidate.critical ? 124 : 103,
			builderCount: candidate.workerCount >= 4 ? 4 : 3,
			builderPool: ["wood", "citizenSoldierWood", "stone", "metal"],
			resourceGeneric: "mixed", resourceAnchor: [...candidate.center], resourceSourceIds: ids,
			resourcePlacementSourceIds: ids,
			minimumAverageDistanceGain: candidate.critical ? Number(policy.resourceForecastStorehouseCriticalMinimumGain) || 2 :
				Number(policy.resourceForecastStorehouseMinimumGain) || 4,
			reason: "forecast mixed-resource service value=" + Math.round(candidate.value) + " gain=" +
				(Number.isFinite(candidate.gain) ? candidate.gain.toFixed(1) : "inf")
		}];
		this.lastForecastStorehouseAt = now;
		aiWarn("[EXPERT-SERVICE] build=storehouse resource=mixed value=" + Math.round(candidate.value) +
			" workers=" + candidate.workerCount + " types=" + candidate.resourceTypes + " gain=" +
			(Number.isFinite(candidate.gain) ? candidate.gain.toFixed(1) : "inf") + " critical=" + candidate.critical);
		return { ...frame, actions };
	}
	...
	huntingCavalryTarget(gameState, cc)
	{
		...
		// IT14.96: wild-game economics outrank doctrine labels. A rich safe hunt is worth
		// the same two-hunter pulse whether the rolled strategy was P1, P2 or P3.
		const two = policy.huntingCavalryMinimumHuntForTwo;
		const three = policy.huntingCavalryMinimumHuntForThree;
	...
	trainExpertHuntingCavalry(gameState, cc)
	{
		...
		// Do not steal the CC from a phase reservation. The hunt pulse is deliberately
		// short (at most two additional hunters) and then civilian continuity resumes.
		...
		if (liveQueue.some(item => item && item.metadata && item.metadata.expertDecisionTraining === "hunt_cavalry"))
			return false;
		...
		if (!selected || !selected.pursuit)
			return false;
		const bank = gameState.getResources();
		const richBatch = need >= 2 && hunt.amount >= (Number(policy.huntingCavalryRichBatchHunt) || 900);
		const foodReserve = richBatch ? (Number(policy.huntingCavalryRichBatchFoodReserve) || 60) :
			(Number(policy.huntingCavalryFoodReserve) || 120);
		const affordableFood = Math.max(0, Math.floor(((Number(bank.food) || 0) - foodReserve) / Math.max(1, selected.cost.food)));
		...
		let batch = Math.min(2, need, affordableFood, affordableWood, affordableStone, affordableMetal);
		...
		aiWarn("[EXPERT-CAV] queued cc-pursuit-hunter=" + selected.type + " trainer=" + cc.id() +
			" batch=" + batch + " hunt=" + Math.round(hunt.amount) + " target=" + hunt.target + " existing=" + existing);
		return true;
	}
	...
	shouldSuppressNormalHouseConstruction(gameState)
	{
		...
		const phase = Number(gameState.currentPhase()) || 1;
		const doctrine = this.ensureStrategicDoctrine(gameState);
		const techDoctrine = doctrine && (doctrine.id === "p2_tech_push" || doctrine.id === "p3_boom_all_in");
		// IT15.6: a tech/boom line may reserve the 13-house breakpoint before Town.
		// 160 cap is enough to reach the phase/tech; houses 14-15 are reserved for a true
		// P1 all-in or emergency where the housing tech cannot arrive in time.
		if (phase < 2)
			return !!(techDoctrine && committedHouses >= mandatoryAt);
	...
	selectInfantrySoldier(gameState, trainer, source = "barracks", resourceBudget = undefined)
	{
		...
		const operatingCap = this.effectiveOperatingPopulationCap(gameState);
		const configuredPoliceHeadroom = Number(policy.athensPoliceActivationPopulationHeadroom);
		const policeHeadroom = Math.max(0, Number.isFinite(configuredPoliceHeadroom) ? configuredPoliceHeadroom : 0);
		const policeAllowed = civ === "athen" && gameState.getPopulation() >= operatingCap - policeHeadroom;
		const candidates = [];
		for (const type of trainer.trainableEntities(civ) || [])
		{
			const typeName = String(type).toLowerCase();
			const cityPolice = civ === "athen" && typeName.includes("city_police");
			// IT14.89: Scythian civic police cost zero population, but are deliberately
			// weak support troops. Keep them out of normal composition until Athens is
			// genuinely at its operating population ceiling; then they become bonus bodies.
			if (cityPolice && !policeAllowed)
				continue;
			if (gameState.isTemplateDisabled(type))
				continue;
			const template = gameState.getTemplate(type);
			...
			candidates.push({ type, template, cost: { food, wood, stone, metal }, melee, ranged, hoplite, javelineer, slinger, swordsman, cityPolice, speed, costScore });
		}
		if (!candidates.length)
			return undefined;
		...
		if (policeAllowed && source === "cc-cap")
		{
			const police = candidates.filter(candidate => candidate.cityPolice);
			if (police.length)
				return chooseAffordable(police, candidates);
		}
	...
	queueExpertCivilianContinuity(gameState, queues, cc)
	{
		...
		let huntPulseNeeded = false;
		if (gameState.getPopulation() >= (Number(policy.huntingCavalryPopulation) || 30) &&
		    (Number(gameState.ai.elapsedTime) || 0) >= (Number(policy.huntingCavalryCCMinimumTime) || 120))
		{
			const hunt = this.huntingCavalryTarget(gameState, cc);
			let existingCavalry = 0;
			for (const ent of gameState.getOwnUnits().values())
		...
		// IT15.8.16: use the real 50-food unit cost once the opening is stable. The old
		// 350/700 safety bands committed another batch of three even when the UI could
		// already afford a fourth or fifth civilian.
		let batch = gameState.getPopulation() < 24 ? 3 : food >= 250 ? 5 : food >= 200 ? 4 : food >= 150 ? 3 : food >= 50 ? 2 : 1;
	...
	pruneUnfundedStandbySoldierPlans(gameState, queues, trainer)
	{
		const queue = queues && queues.citizenSoldier;
		if (!queue || !Array.isArray(queue.plans) || !trainer)
			return false;
		const bank = gameState.getResources();
		const liveBudget = {
			"food": Number(bank.food) || 0, "wood": Number(bank.wood) || 0,
			"stone": Number(bank.stone) || 0, "metal": Number(bank.metal) || 0
		};
		const fallback = this.selectInfantrySoldier(gameState, trainer, "barracks-auto", liveBudget);
		...
		let pruned = false;
		queue.plans = queue.plans.filter(plan =>
		{
			if (!plan || !plan.metadata || Number(plan.metadata.trainer) !== trainer.id() ||
			    plan.metadata.expertDecisionStandby !== true || !plan.type)
				return true;
			const template = gameState.getTemplate(plan.type);
			const cost = template && template.cost ? template.cost(trainer) : undefined;
			if (!cost)
				return true;
			const funded = liveBudget.food >= (Number(cost.food) || 0) && liveBudget.wood >= (Number(cost.wood) || 0) &&
				liveBudget.stone >= (Number(cost.stone) || 0) && liveBudget.metal >= (Number(cost.metal) || 0);
			if (funded || plan.type === fallback.type)
				return true;
			pruned = true;
			aiWarn("[EXPERT-TRAIN] replace-standby trainer=" + trainer.id() + " old=" + plan.type +
				" substitute=" + fallback.type + " bank=" + Math.round(liveBudget.food) + "/" +
				Math.round(liveBudget.wood) + "/" + Math.round(liveBudget.stone) + "/" + Math.round(liveBudget.metal));
			return false;
		});
		return pruned;
	}
	...
	queueExpertSoldierBatch(gameState, queues, trainer, source, requestedBatch = 2, maxWorkDepth = 1, allowStandby = false)
	{
		...
		const protectWood = this.phaseWoodCrisis ||
			(this.strategyWoodPivot && gameState.currentPhase && gameState.currentPhase() === 1) ||
			(gameState.getPlayerCiv() === "athen" && (this.woodIncomeStalled || athensResourceSlingerWindow));
		const liveWood = Number(resources.wood) || 0;
		// IT15.0 no-idle production: protecting wood means keeping a reserve, not pretending
		...
		// IT15.0 fundamental production rule: a preference/reserve may change WHICH unit
		// we want, but it may not leave a completed trainer empty while the live bank can
		// afford any valid citizen-soldier. Re-run selection against the unreserved bank
		// and immediately substitute one affordable Hoplite/Javelineer/etc.
		...
		aiWarn("[EXPERT-MIL] queued " + source + " soldiers=" + selected.type + " batch=" + batch + " trainer=" + trainer.id() +
			...
			(noIdleFallback ? " mode=no-idle-substitute" : "") +
	...
	trainExpertMilitary(gameState, queues, cc)
	{
		...
		const food = Number(gameState.getResources().food) || 0;
		// IT15.3: Barracks use efficient 2-3 soldier batches like a human player.
		// queueExpertSoldierBatch() still shrinks to one immediately when only one is
		// affordable, so batching can never become a new trainer-idle condition.
		const militaryBatch = food >= 900 ? 3 : Math.max(2, Math.min(3, Number(policy.soldierTrainingBatch) || 2));
		...
		{
			const now = Number(gameState.ai.elapsedTime) || 0;
			// If a standby Swordsman is waiting for metal while a Hoplite/Javelineer is
			// affordable now, delete the stale preference and refill immediately.
			this.pruneUnfundedStandbySoldierPlans(gameState, queues, barracks);
		...
		if (gameState.getPlayerCiv() === "athen")
		{
			...
			{
				const depth = Math.max(2, Number(policy.expertCleruchyProductionQueueDepth) || 2);
				const batch = Math.max(2, Math.min(3, militaryBatch, Number(policy.expertCleruchyTrainingBatch) || 2));
				this.pruneUnfundedStandbySoldierPlans(gameState, queues, cleruchy);
				const now = Number(gameState.ai.elapsedTime) || 0;
				const workDepth = this.expertSoldierWorkCount(queues, cleruchy);
				if (workDepth > 0)
					delete this.trainerIdleSince[cleruchy.id()];
				else if (!Number.isFinite(Number(this.trainerIdleSince[cleruchy.id()])))
					this.trainerIdleSince[cleruchy.id()] = now;
				if (this.queueExpertSoldierBatch(gameState, queues, cleruchy, "cleruchy-auto", batch, depth, true))
				{
					const since = Number(this.trainerIdleSince[cleruchy.id()]);
					const gap = Number.isFinite(since) ? now - since : 0;
					if (gap > 2)
						aiWarn("[EXPERT-TRAIN] cleruchy-gap trainer=" + cleruchy.id() + " seconds=" + gap.toFixed(1));
					aiWarn("[EXPERT-TRAIN] cleruchy-auto trainer=" + cleruchy.id() + " batch=" + batch + " depth=" +
	...
	placementRequest(gameState, action, cc, accessIndex, foodObservation)
	{
		...
		else if (kind === "storehouse" && action.role === "resource_service")
		{
			...
			const placementIds = Array.isArray(action.resourcePlacementSourceIds) && action.resourcePlacementSourceIds.length ?
				action.resourcePlacementSourceIds : sourceIds;
			const sources = sourceIds.map(id => gameState.getEntityById(Number(id))).filter(ent => ent && entityPosition(ent));
			let resourcePoints = this.resourcePlacementPoints(gameState, placementIds);
			const massCenter = this.weightedResourceCenter(resourcePoints);
			const anchor = massCenter || (Array.isArray(action.resourceAnchor) ? action.resourceAnchor : cc.position());
			...
			const distances = woodService ?
				...
				[Math.max(3, geometry.radius + 0.5), geometry.radius + 1.5, geometry.radius + 2.5, geometry.radius + 4.0, geometry.radius + 6.0, geometry.radius + 8.0];
			const candidates = [];
			for (const candidateAnchor of candidateAnchors)
				candidates.push(...generatePlacementCandidates({
					"kind": "storehouse", "anchor": candidateAnchor, "toward": cc.position(),
					distances, "angleCount": woodService ? 48 : 32, "templateRadius": geometry.radius
				}));
			const emergency = woodService && (this.woodIncomeStalled || this.phaseWoodCrisis ||
				((Number(gameState.getResources().wood) || 0) <= (Number(policy.woodCrisisImmediateBank) || 250) &&
				 this.actualWorkerOrders(gameState).wood <= (Number(policy.woodCrisisImmediateActiveWorkers) || 2)));
			request = {
				...
				"minimumAverageDistanceGain": Number.isFinite(Number(action.minimumAverageDistanceGain)) ? Number(action.minimumAverageDistanceGain) : woodService ?
					(emergency ? Number(policy.storehouseEmergencyMinimumAverageDistanceGain) || 2 : Number(policy.storehouseMinimumAverageDistanceGain) || 5) :
					Number(policy.storehouseMineralMinimumAverageDistanceGain) || 4,
		...
		else if (kind === "storehouse")
		{
			...
			const liveFront = this.liveWoodCuttingFront(gameState, accessIndex, current, currentId);
			const districtAnchor = liveFront && liveFront.center ? liveFront.center : current;
			const nearby = collectInitialWoodCandidates(gameState, {
				"getLandAccess": getLandAccess, "isSupplyFull": isSupplyFull,
				"territoryMap": this.HQ.territoryMap, "anchorPosition": districtAnchor,
				"accessIndex": accessIndex, "playerId": PlayerID,
				"searchRadius": Math.max(Number(policy.woodClusterSearchRadius) || 0, Number(policy.woodCuttingFrontSearchRadius) || 82),
				"allowNeutralWood": false
			});
			const currentCluster = this.connectedWoodCluster(nearby, districtAnchor, policy.woodClusterLinkDistance);
			...
			let practicalCluster = currentCluster.length ? nearby.filter(tree => connectedIds.has(tree.id) ||
				currentCluster.some(baseTree => SquareVectorDistance(tree.position, baseTree.position) <= fringeDistance2)) : currentCluster;
			if (liveFront && liveFront.trees && liveFront.trees.length)
			{
				const merged = new Map(practicalCluster.map(tree => [Number(tree.id), tree]));
				for (const tree of liveFront.trees) merged.set(Number(tree.id), tree);
				practicalCluster = [...merged.values()];
			}
			...
			const cuttingFrontAdvance = !!(liveFront &&
				liveFront.workers >= (Number(policy.woodCuttingFrontAdvanceMinimumWorkers) || 8) &&
				liveFront.remaining >= (Number(policy.woodCuttingFrontAdvanceMinimumRemaining) || 450) &&
				liveFront.averageDropDistance >= (Number(policy.woodCuttingFrontAdvanceMinimumDropDistance) || 14));
			let ranked = [];
			let mode = "new_patch";
			let improvement = 0;

			if (practicalCluster.length && ((localWorkers.length >= requiredWorkers && samePatchAmount >= requiredWood) || lateSamePatchReuse || cuttingFrontAdvance))
			{
			...
			if (!ranked.length)
			{
				// IT14.89: "finish this district" becomes a real hold only when LIVE evidence
				// says the current district is healthy: plenty of wood, a serving dropsite,
				// multiple active cutters, and no measured stall/crisis. This is intentionally
				// different from IT14.86's graph-connectivity veto; the moment delivery/active
				// cutters collapse, continuityEmergency/primary health releases the next forest.
				const districtHealth = this.primaryWoodDistrictHealth(gameState, accessIndex);
				if (!continuityEmergency && districtHealth.healthy)
				{
					if ((Number(gameState.ai.elapsedTime) || 0) - (Number(this.lastWoodDriftHoldDiag) || -99999) >= 8)
					{
						this.lastWoodDriftHoldDiag = Number(gameState.ai.elapsedTime) || 0;
						aiWarn("[EXPERT-WOOD] HOLD healthy serviced district wood=" + Math.round(districtHealth.remaining) +
							" active=" + districtHealth.activeWorkers + " stores=" + districtHealth.stores + " action=no-new-forest");
					}
					return undefined;
				}
				if (practicalCluster.length)
					aiWarn("[EXPERT-WOOD] release serviced district practicalWood=" + Math.round(samePatchAmount) +
						" active=" + districtHealth.activeWorkers + " emergency=" + continuityEmergency);

				// If the practical district cannot be improved AND is no longer economically
				// healthy, search a real in-territory forest. Crisis still widens the search.
			...
			const selection = selectInitialWoodWorksite(all, workerAnchor, {
				...
				if (!selection || !selection.position)
					return undefined;
				const forecastWood = this.resourceForecast && this.resourceForecast.resources && this.resourceForecast.resources.wood;
				const bridgeWood = !!(forecastWood && forecastWood.status === "critical" &&
					forecastWood.ownAccessible <= (Number(policy.resourceForecastBridgeWoodAccessibleCeiling) || 650));
				const minimumDistrictWood = bridgeWood ?
					Math.max(1, Number(policy.resourceForecastBridgeWoodMinimumAmount) || 120) : continuityEmergency ?
				...
				if (!ranked.length)
				{
					aiWarn("[EXPERT-WOOD] reject fragment district bestWood=" + Math.round(Number(selection.localWoodAmount) || 0) +
						" forest=" + Math.round(Number(selection.forestWoodAmount || selection.localWoodAmount) || 0) +
						" minimum=" + Math.round(minimumDistrictWood) + " emergency=" + continuityEmergency + " forecastBridge=" + bridgeWood);
					return undefined;
				}
			}
			...
			if (!candidates.length)
				return undefined;
			const resourcePoints = this.resourcePlacementPoints(gameState, ranked[0].treeIds || []);
			request = {
				kind, "templateRadius": geometry.radius, candidates,
				"worksiteAnchor": ranked[0].position, "selectedTreeIds": [...(ranked[0].treeIds || [])],
				"resourceGeneric": "wood", "resourcePoints": resourcePoints,
				"minimumAverageDistanceGain": continuityEmergency ?
					(Number(policy.storehouseEmergencyMinimumAverageDistanceGain) || 2) :
					(Number(policy.storehouseMinimumAverageDistanceGain) || 5),
			...
			aiWarn("[EXPERT-WOOD] storehouse plan=" + mode + " currentWood=" + Math.round(samePatchAmount) +
				" candidateWood=" + Math.round(Number(ranked[0].localWoodAmount) || 0) +
				" workers=" + localWorkers.length + " stores=" + servingStores + " anchorImprove=" + improvement.toFixed(1));
		}
		...
		else if (kind === "farmstead")
		{
			...
			if (action.role === "natural_expansion" || action.role === "natural_frontier" || action.role === "wicker_branch" || action.role === "resource_service_food")
			{
				const foodContext = this.foodCaptureContext(gameState, cc, accessIndex);
				const alternative = (action.role === "resource_service_food" || action.role === "natural_frontier") && Array.isArray(action.resourceAnchor) ?
					{ "center": action.resourceAnchor, "ids": Array.isArray(action.resourceSourceIds) ? action.resourceSourceIds : [],
						"remaining": Array.isArray(action.resourceSourceIds) ? action.resourceSourceIds.reduce((sum, id) => {
							const ent = gameState.getEntityById(Number(id));
							return sum + Math.max(0, Number(ent && ent.resourceSupplyAmount && ent.resourceSupplyAmount()) || 0);
						}, 0) : 0 } :
			...
			{
				// Completion must hand the same four opening builders directly to these
				// supplies. Without this task-to-cluster record the generic construction
				// release path forgets why they built the Farmstead and may send them to wood.
				this.pendingFoodSelectionByTask[taskId] = {
					"center": Array.isArray(foodObservation.center) ? [...foodObservation.center] : anchor,
					"ids": [...sourceIds],
					"remaining": Math.max(0, Number(foodObservation.remaining) || 0)
				};
		...
		else if (kind === "house")
		{
			...
			else
			{
				...
				// IT14.96 compact housing: continue a dense block instead of scattering Houses
				// into tiny three-house islands. Legal obstruction spacing still owns the final veto;
				// the snap candidates simply ask for the minimum practical edge gap.
				...
				// IT14.89: first try to fill a literal one-house hole between existing blocks.
				// We derive these from the same snap geometry, so this does not invent a new
				// city layout; it simply uses the missing shared-adjacent slot before spreading.
				const gapFillHouseCandidates = [];
				const adjacentProbe = [];
				for (const anchorHouse of houses)
				{
					...
					adjacentProbe.push(expertLocalToWorld(pos, +snapU, 0, angle));
					adjacentProbe.push(expertLocalToWorld(pos, -snapU, 0, angle));
					adjacentProbe.push(expertLocalToWorld(pos, 0, +snapV, angle));
					adjacentProbe.push(expertLocalToWorld(pos, 0, -snapV, angle));
				}
				const gapTolerance = Math.max(0.5, Number(policy.houseGapFillSnapTolerance) || 2.5);
				for (const candidate of adjacentProbe)
				{
					let adjacent = 0;
					for (const house of houses)
					{
						const d = Math.sqrt(SquareVectorDistance(candidate, house.position()));
						if (Math.abs(d - snapU) <= gapTolerance || Math.abs(d - snapV) <= gapTolerance)
							++adjacent;
					}
					if (adjacent >= 2) gapFillHouseCandidates.push(candidate);
				}
		...
		else if (kind === "temple")
		{
			...
			if (!this.HQ.canBuild || !templeTypes.some(type => this.HQ.canBuild(gameState, type)))
				return undefined;
			const candidates = [];
			const templePipeline = this.builtByClass(gameState, "Temple").length + this.foundationsByClass(gameState, "Temple").length;
			const firstTemple = templePipeline === 0;
			...
			request = { kind, candidates, "templateRadius": geometry.radius,
				...
				"maximumCCDistance": firstTemple ? (Number(policy.firstTempleMaximumCCDistance) || 52) : 0,
				"preferredCCDistance": firstTemple ? (Number(policy.firstTemplePreferredCCDistance) || 30) : 0,
				"firstTempleCentral": firstTemple,
	...
	placementPorts(gameState, kind, accessIndex)
	{
		...
		ports.extraValidation = (position, request) =>
		{
			...
			if (request && Number(request.maximumCCDistance) > 0 && (kind === "house" || kind === "temple"))
			{
				const coreCC = this.findCC(gameState);
				const maxCC = Number(request.maximumCCDistance);
			...
			{
				const spacing = request && request.resourceGeneric === "mixed" ?
					(Number(mergePolicy().resourceForecastStorehouseMinimumSpacing) || 12) : request && request.resourceService ?
				...
				for (const ent of this.builtByClass(gameState, "Storehouse"))
					if (SquareVectorDistance(position, ent.position()) < spacing * spacing)
						return false;
				// IT14.89 unified payback gate. Every non-opening Storehouse request that
				// knows its resource mass must shorten the weighted average haul by a real
				// amount. This applies equally to wood, stone and metal request paths.
				const minimumGain = Math.max(0, Number(request && request.minimumAverageDistanceGain) || 0);
				if (minimumGain > 0 && request && Array.isArray(request.resourcePoints) && request.resourcePoints.length)
				{
					const gain = this.storehouseAverageDistanceGain(gameState, request.resourceGeneric || "wood", request.resourcePoints, position);
					if (Number.isFinite(gain) && gain < minimumGain)
						return false;
				}
			}
		...
		if (kind === "storehouse")
			ports.scoreCandidate = (position, request, index) =>
			{
				...
				if (request && request.openingStorehouse)
				{
					...
					if (points.length)
					{
						...
						for (const point of points)
						{
							if (!point || !Array.isArray(point.position)) continue;
							const weight = Math.max(0.001, Number(point.weight) || 1);
							totalWeight += weight;
							distanceScore += Math.sqrt(SquareVectorDistance(point.position, position)) * weight;
						}
						if (totalWeight > 0)
							score += (Number(policy.openingStorehouseWoodDistanceScoreWeight) || 180) * distanceScore / totalWeight;
					}
					if (Array.isArray(request.foodDistrictAnchor))
					{
						...
						if (distance < radius)
							score += (Number(policy.openingStorehouseFoodDistrictPenalty) || 160) * (radius - distance);
					}
				...
				if (!request)
					return score;
				// IT14.89: put the Storehouse beside the RESOURCE MASS, not beside the first
				// target in a worker list. This fixes fringe-tree lumber camps and naturally
				// centers mineral Storehouses on the mine(s) they are meant to service.
				const points = Array.isArray(request.resourcePoints) ? request.resourcePoints : [];
				if (points.length)
				{
					let totalWeight = 0, distanceScore = 0, obstructionScore = 0;
					const obstructionSample = points.slice(0, 12);
					for (const point of points)
					{
						if (!point || !Array.isArray(point.position)) continue;
						const weight = Math.max(0.001, Number(point.weight) || 1);
					...
					let obstructionWeight = 0;
					for (const point of obstructionSample)
					{
						const weight = Math.max(0.001, Number(point.weight) || 1);
						obstructionWeight += weight;
						obstructionScore += this.lineObstructionPenalty(ports.obstructionMap, point.position, position) * weight;
					}
					const policy = mergePolicy();
					if (totalWeight > 0)
						score += (Number(policy.storehouseResourceDistanceScoreWeight) || 140) * distanceScore / totalWeight;
					if (obstructionWeight > 0)
						score += (Number(policy.storehouseResourceObstructionScoreWeight) || 30) * obstructionScore / obstructionWeight;
					if (Array.isArray(request.worksiteAnchor))
						score += 8 * Math.sqrt(SquareVectorDistance(request.worksiteAnchor, position));
					return score;
				}
				// Legacy fallback for unusual requests that do not provide resource points.
			...
			{
				let score = Number(index) || 0;
				if (request && request.openingHouse)
				{
					const coreCC = this.findCC(gameState);
					if (coreCC && entityPosition(coreCC))
						score += 45 * Math.abs(Math.sqrt(SquareVectorDistance(position, coreCC.position())) - (Number(request.preferredCCDistance) || 38));
					if (Array.isArray(request.openingFoodDistrictAnchor))
					{
						const radius = Number(request.openingFoodDistrictReserveRadius) || 34;
						const distance = Math.sqrt(SquareVectorDistance(position, request.openingFoodDistrictAnchor));
						if (distance < radius + 10)
							score += (Number(mergePolicy().openingHouseFoodDistrictPenalty) || 12000) * Math.max(0, radius + 10 - distance);
					}
			...
			ports.scoreCandidate = (position, request, index) =>
			{
				...
				const r2 = radius * radius;
				let covered = 0, mineralCovered = 0, farmCovered = 0;
				for (const worker of gameState.getOwnUnits().values())
					if (worker && entityPosition(worker) && hasClass(worker, "Worker") &&
					...
					{
						++covered;
						const job = worker.getMetadata ? worker.getMetadata(PlayerID, JOB_METADATA) : undefined;
						if (job === "stone" || job === "metal") ++mineralCovered;
						if (job === "farm" || job === "food" || job === "food_owned") ++farmCovered;
					}
				let score = (Number(index) || 0) - covered * 9000 - mineralCovered * 3500 - farmCovered * 1200;
				if (request && request.firstTempleCentral)
				{
					const coreCC = this.findCC(gameState);
					if (coreCC && entityPosition(coreCC))
						score += 80 * Math.abs(Math.sqrt(SquareVectorDistance(position, coreCC.position())) - (Number(request.preferredCCDistance) || 30));
				}
				return score;
			};
		else if (kind === "market")
		...
		for (const action of orderedActions)
		{
			if (action.type === "BUILD")
			{
				...
				if (action.kind === "field" && Array.isArray(action.requiredBuilderIds))
					...
					{
						const worker = gameState.getEntityById(Number(workerId));
						if (worker && worker.getMetadata && worker.setMetadata &&
						    worker.getMetadata(PlayerID, EXPERT_FUTURE_FARMER) === true)
							worker.setMetadata(PlayerID, EXPERT_FUTURE_FARM_TASK, exec.taskId);
					}
				if (action.kind === "house")
				{
					...
					{
						const footprint = 2 * Math.max(Number(half.width) || houseGeom.radius, Number(half.depth) || houseGeom.radius);
						const edgeGap = Math.max(0, nearest - footprint);
						aiWarn("[EXPERT-HOUSE-SNAP] task=" + exec.taskId + " beside=" + nearestId +
							" center=" + nearest.toFixed(1) + " edgeGap=" + edgeGap.toFixed(2));
					}
				}
	...
	applySecondaryDepletionFieldTrigger(gameState, frame)
	{
		...
		if (natural && Math.max(0, Number(natural.totalNaturalRemaining) || 0) > 0 &&
		    Number(natural.territoryNaturalRatio) > Number(mergePolicy().territoryNaturalFarmTransitionRatio || 0.25))
			return frame;
	...
	assignFoodHomeLocalWork(gameState, ent, home, accessIndex)
	{
		if (!ent || !entityPosition(ent) || !home || !entityPosition(home))
			return false;
		const radius = Math.max(30, Number(mergePolicy().farmWorkerHomeRadius) || 55);
		// IT15.1 Steven labor contract: food-home civilians are the permanent food
		// backbone. They do not become local lumberjacks/miners when a Field is missing.
		for (const generic of ["food"])
		{
			let source = gameState.getResourceSupplies ?
				[...gameState.getResourceSupplies("food").values()] : [];
			let candidates = source.filter(supply =>
			{
				const pos = entityPosition(supply);
				if (!pos || getLandAccess(gameState, supply) !== accessIndex || SquareVectorDistance(pos, home.position()) > radius * radius)
					return false;
				if (generic === "food")
				{
					if (hasClass(supply, "Animal") || hasClass(supply, "Field"))
						return false;
					const owner = this.HQ.territoryMap.getOwner(pos);
					if (owner !== PlayerID)
						return false;
				}
				return !supply.resourceSupplyAmount || supply.resourceSupplyAmount() > 0;
			});
			if (generic === "food")
				candidates = candidates.filter(supply => !isSupplyFull(gameState, supply));
			if (!candidates.length)
				continue;
			candidates.sort((a, b) =>
				...
				a.id() - b.id());
			const target = candidates[0];
			if (this.depositBeforeResourceRetarget(gameState, ent, generic, "food-home-local"))
				return true;
			ent.setMetadata(PlayerID, JOB_METADATA, generic === "food" ? "food_owned" : "food_owned");
			ent.setMetadata(PlayerID, SUPPLY_ID, target.id());
			ent.setMetadata(PlayerID, "gather-type", generic);
			ent.setMetadata(PlayerID, "subrole", Worker.SUBROLE_GATHERER);
	...
	assignFoodWorker(gameState, ent, foodNetwork, accessIndex)
	{
		...
		if (!lockedCluster && ent.getMetadata(PlayerID, FOOD_HOME_PERMANENT) === true)
		{
			...
			{
				// IT15.0 locality contract: a civilian that built/owns this food district
				// gathers the natural food beside it before any secondary-resource fallback.
				// This is deliberately checked BEFORE fields because the Farmstead was paid
				// for to service this natural cluster in the first place.
				...
				else
				{
					...
					// IT15.0: never solve a local food-capacity wait by sending this worker
					// across the map to a distant woodline. A local fallback is allowed;
					// otherwise the field/hub planner owns the next assignment.
		...
		const now = Number(gameState.ai.elapsedTime) || 0;
		if (!lockedCluster && cluster && !this.foodClusterAssignmentServiced(gameState, cluster) &&
		    this.foodClusterFarmsteadWorthwhile(gameState, cluster))
		{
			// A roster/site hint is not permission to long-haul to an uncovered patch.
			// Keep the civilian productive locally until the branch Farmstead foundation
			// turns that patch into a real food district.
			cluster = undefined;
			ent.setMetadata(PlayerID, FOOD_SITE, undefined);
			ent.setMetadata(PlayerID, SUPPLY_ID, undefined);
		}
		...
		let ranked = lockedCluster ? [lockedCluster] : clusters.filter(candidate =>
			...
			(this.foodClusterAssignmentServiced(gameState, candidate) || !this.foodClusterFarmsteadWorthwhile(gameState, candidate)) &&
			...
			!this.naturalFoodClusterHasPreferredSlot(gameState, cluster, ent))))
		{
			...
			if (rushFoodBackbone)
			{
				// IT15.2: food ownership is permanent, idling is not. If the permanent-food
				// network is temporarily saturated, keep this civilian productive on an owned
				// serviced resource while the Field/Farmstead pipeline catches up. JOB_METADATA
				// remains food_owned, so food is retried every Expert update.
				...
				this.diagnoseWorkerOrder(ent, "food-capacity-miss", 0, fallbackAssigned ?
					"FOOD_CAPACITY_DEBT_TEMP_WORK" : "FOOD_CAPACITY_DEBT_NO_TARGET");
	...
	assignFoodInfrastructureWorker(gameState, ent)
	{
		if (!ent || !entityPosition(ent))
			return false;
		const futureFarmer = ent.getMetadata && ent.getMetadata(PlayerID, EXPERT_FUTURE_FARMER) === true;
		const foodJob = ent.getMetadata && ent.getMetadata(PlayerID, JOB_METADATA);
		const foodInfrastructureEligible = futureFarmer || ["food", "food_owned", "farm"].includes(foodJob);
		const policy = mergePolicy();
		...
		const homePosition = homeFarmstead && entityPosition(homeFarmstead);
		const naturalTransitionEmergency = Number(this.lastTerritoryNaturalFoodRatio) <=
			Number(policy.territoryNaturalFarmTransitionRatio || 0.25) &&
			this.builtByClass(gameState, "Field").length < Math.max(6, Number(policy.minimumCompletedFieldsBeforeSecondBarracks) || 6);
		const fieldFoundations = foodInfrastructureEligible ? this.foundationsByClass(gameState, "Field").filter(foundation =>
		{
			if (!foundation || !entityPosition(foundation)) return false;
			const taskId = foundation.getMetadata && foundation.getMetadata(PlayerID, "expertTaskId");
			if (!taskId) return false;
			const intent = this.activeTaskBuildIntent[taskId] || {};
			const required = Array.isArray(intent.requiredBuilderIds) ? intent.requiredBuilderIds.map(Number) : [];
			const localHomeFoundation = !!(homePosition &&
				SquareVectorDistance(foundation.position(), homePosition) <= Math.pow(Math.max(30, Number(policy.farmWorkerHomeRadius) || 55), 2));
			// IT15.8.13: requiredBuilderIds selects the workers that PLACE a Field; it
			// must not exclude the exhausted berry crew from helping construct that Field.
			// During the six-Field emergency, any food-owned civilian may reinforce it,
			// with the local Farmstead crew taking priority through normal roster order.
			if (required.length && !required.includes(ent.id()) && !localHomeFoundation && !naturalTransitionEmergency)
				return false;
			const desired = Math.max(1, Number(intent.builderCount) || Number(mergePolicy().futureFarmFieldBuilderTarget) || 4);
			const existing = this.constructionWorkers(gameState, taskId);
			const emergencyDesired = naturalTransitionEmergency ?
	...
	workerWoodsite(gameState, ent, primaryWoodsite, accessIndex)
	{
		...
		// IT14.92: do not erase the Storehouse assignment simply because the live
		// cutting edge has moved beyond the old fixed-radius ring. Follow the workers'
		// real tree targets and their nearby continuation first; this is what lets the
		// dropsite advance along one forest instead of marching the crew to another.
		const liveFront = entityId !== undefined ? this.liveWoodCuttingFront(gameState, accessIndex, position, entityId) : undefined;
		if (liveFront && liveFront.trees && liveFront.trees.length && liveFront.remaining > 0)
		{
			const liveMetrics = summarizeWoodTrees(liveFront.trees);
			return { trees: liveFront.trees, ...liveMetrics, position, entityId, liveFront: true };
		}
	...
	activeResourceDistricts(gameState, accessIndex)
	{
		...
		for (const district of districts)
		{
			...
			district.placementSourceIds = [...district.sourceIds];
			// IT14.89: workers usually target only a handful of trees at once. Expand the
			// ACTUAL worker district into the connected/practical forest and use that wood
			// mass for placement too. This prevents a Storehouse from anchoring on one fringe
			// tree while the real forest is several metres farther away.
			if (district.generic === "wood" && district.center)
			{
				const radius = Math.max(30, Number(policy.woodServicePlacementSearchRadius) || 48);
				const nearbyTrees = collectInitialWoodCandidates(gameState, {
					"getLandAccess": getLandAccess, "isSupplyFull": isSupplyFull,
				...
				const connected = this.connectedWoodCluster(nearbyTrees, district.center, Number(policy.woodClusterLinkDistance) || 22);
				const fringe2 = Math.pow(Math.max(Number(policy.woodPracticalDistrictFringeDistance) || 34,
					Number(policy.woodClusterLinkDistance) || 22), 2);
				const connectedIds = new Set(connected.map(tree => tree.id));
				const localTrees = connected.length ? nearbyTrees.filter(tree => connectedIds.has(tree.id) ||
					connected.some(base => SquareVectorDistance(tree.position, base.position) <= fringe2)) : connected;
				if (localTrees.length)
				{
					district.remaining = Math.max(district.remaining, this.woodAmount(localTrees));
					district.placementSourceIds = localTrees.map(tree => tree.id);
					const massCenter = this.weightedResourceCenter(localTrees.map(tree => ({ position: tree.position, weight: tree.remaining })));
					if (massCenter) district.center = massCenter;
				}
			}
	...
	resourceServiceNeed(gameState, accessIndex)
	{
		...
		const primaryWood = this.primaryWoodDistrictHealth(gameState, accessIndex);
		const primaryWoodIds = new Set(primaryWood.treeIds || []);
		const candidates = this.activeResourceDistricts(gameState, accessIndex).filter(district =>
		{
			...
			{
				// IT14.89: do not let three wandering lumberjacks "vote" for a new forest
				// while the established district is still serviced, productive and rich.
				// This is deliberately NOT the old IT14.86 hard veto: measured stall/low
				// activity makes primaryWood.healthy false and releases the alternate immediately.
				if (primaryWood.healthy)
				{
					const placementIds = district.placementSourceIds || district.sourceIds || [];
						...
						if (!sameDistrict && district.dropDistance <= 35)
					{
						...
						{
							this.lastWoodDriftHoldDiag = now;
							aiWarn("[EXPERT-WOOD] hold drift district workers=" + district.workers.length +
								" alternateWood=" + Math.round(district.remaining) + " primaryWood=" + Math.round(primaryWood.remaining) +
								" primaryActive=" + primaryWood.activeWorkers);
						}
						return false;
					}
				}
	...
	applyResourceServiceConstruction(gameState, frame, accessIndex)
	{
		...
		const action = {
			...
			"resourcePlacementSourceIds": [...(need.placementSourceIds || need.sourceIds)],
		...
		if (generic === "wood" && !out.length)
		{
			...
			if (this.woodIncomeStalled || forecastWood && forecastWood.status === "critical")
			{
				...
				for (const supply of emergencyWood)
				{
					let best = Infinity;
					for (const site of dropsites)
						best = Math.min(best, SquareVectorDistance(supply.position(), site.position()));
	...
	depositBeforeResourceRetarget(gameState, ent, targetGeneric, label)
	{
		...
		// Only a REAL return order counts as productive work. If no dropsite exists,
		// continue below and issue a gather order rather than parking the worker idle.
		return !!queued;
	}
	...
	assignSafeFallback(gameState, ent, accessIndex, preferred = ["wood", "food", "stone", "metal"], failedTargetId = undefined)
	{
		...
		for (const generic of preferred)
		{
			...
			for (const target of candidates.slice(0, 8))
			{
				...
				{
					ent.setMetadata(PlayerID, EXPERT_FALLBACK_FAILED_TARGET, target.id());
					continue;
				}
	...
	assignIdleConstruction(gameState, ent, accessIndex)
	{
		if (!ent || !entityPosition(ent) || typeof ent.repair !== "function")
			return false;
		const kinds = ["Storehouse", "Barracks", "Forge", "Farmstead", "Field", "Market", "House", "Tower", "Temple", "Arsenal"];
		const candidates = [];
		for (const kind of kinds)
			for (const foundation of this.foundationsByClass(gameState, kind))
			{
				if (!foundation || !entityPosition(foundation) || getLandAccess(gameState, foundation) !== accessIndex ||
				    this.HQ.territoryMap.getOwner(foundation.position()) !== PlayerID)
					continue;
				const distance = SquareVectorDistance(ent.position(), foundation.position());
				if (distance > 85 * 85)
					continue;
				let workers = 0;
				for (const unit of gameState.getOwnUnits().values())
					if (unit && hasLiveRepairOrder(unit, foundation.id()))
						++workers;
				if (workers < 6)
					candidates.push({ foundation, distance, workers });
			}
		candidates.sort((a, b) => a.workers - b.workers || a.distance - b.distance ||
			a.foundation.id() - b.foundation.id());
		for (const candidate of candidates)
		{
			const order = ensureRepairOrder(ent, candidate.foundation, false);
			if (order.status === "FAILED")
				continue;
			ent.setMetadata(PlayerID, "subrole", Worker.SUBROLE_BUILDER);
			aiWarn("[EXPERT-NO-IDLE] construct worker=" + ent.id() +
				" foundation=" + candidate.foundation.id() + " status=" + order.status);
			return true;
		}
		return false;
	}
	...
	enforceNoIdleEconomyWorkers(gameState, accessIndex, foodNetwork)
	{
		...
		for (const ent of gameState.getOwnUnits().values())
		{
			    ...
			    ent.getMetadata(PlayerID, EXPERT_FUTURE_FARMER) === true)
			{
				...
				const homeId = Number(ent.getMetadata(PlayerID, FOOD_HOME_FARMSTEAD));
				const home = Number.isFinite(homeId) ? gameState.getEntityById(homeId) : undefined;
				if (home && (ent.getMetadata(PlayerID, FOOD_HOME_PERMANENT) === true ||
				             decodeFoodSite(ent.getMetadata(PlayerID, NATURAL_FOOD_LOCK)).length > 0))
				{
					if (this.assignFoodHomeLocalWork(gameState, ent, home, accessIndex))
						continue;
				...
				if (rushFoodBackbone)
				{
					// IT15.2 hard invariant: food-capacity debt may NEVER mean idle. Preserve
					// food_owned metadata but use temporary owned-territory work until a food
					// slot/foundation exists. The next Expert update retries food first.
					const tempWork = this.assignSafeFallback(gameState, ent, accessIndex, ["wood", "stone", "metal"], failedTarget);
					this.diagnoseWorkerOrder(ent, "food-capacity-debt", 0, tempWork ?
						"TEMP_PRODUCTIVE_WORK" : "NO_LEGAL_TEMP_TARGET");
					if (tempWork)
					{
						aiWarn("[EXPERT-NO-IDLE] food-capacity-debt worker=" + ent.id() +
							" temporary=productive-owned-work");
						continue;
					}
				}
			}
			...
			if (!preferred.length)
			{
				...
				// The forecast may rank resources, never veto ALL productive work.
				preferred = broadFallback;
				if (now - (Number(this.lastUsefulWaitDiag) || -99999) >= (Number(policy.resourceForecastUsefulWaitDiagnosticSeconds) || 8))
				{
					this.lastUsefulWaitDiag = now;
					aiWarn("[EXPERT-NO-IDLE] surplus-override worker=" + ent.id() + " job=" + (job || "-") + " preferred=" + preferred.join(">"));
				}
			}
			...
			// Last resort: the preferred economy can be temporarily impossible on sparse
			// maps or saturated mines. Try every legal gather class before accepting idle.
			const rescue = ["wood", "food", "stone", "metal"].filter(type => !preferred.includes(type)).concat(preferred);
			if (this.assignSafeFallback(gameState, ent, accessIndex, rescue, failedTarget))
			{
				aiWarn("[EXPERT-NO-IDLE] ULTIMATE worker=" + ent.id() + " job=" + (job || "-") +
					" rescue=" + rescue.join(">") + " failures=" + failures);
				continue;
			}
			if (this.assignIdleConstruction(gameState, ent, accessIndex))
				continue;
	...
	captureOpeningChickens(gameState, cc, accessIndex)
	{
		...
		for (const supply of gameState.getResourceSupplies("food").values())
		{
			...
			if (!pos || !hasClass(supply, "Animal") || getLandAccess(gameState, supply) !== accessIndex ||
			    this.HQ.territoryMap.getOwner(pos) !== PlayerID)
				continue;
	...
	updateWorkers(gameState, cc, foodNetwork, woodsite, accessIndex)
	{
		for (const ent of gameState.getOwnUnits().values())
		{
			     ...
			     hasClass(ent, "CitizenSoldier") && !hasClass(ent, "Cavalry") && liveSupplyType.generic === "food"))
			{
				...
				ent.setMetadata(PlayerID, EXPERT_FALLBACK_LEASE_RESOURCE, undefined);
				ent.setMetadata(PlayerID, EXPERT_FALLBACK_LEASE_UNTIL, undefined);
				aiWarn("[EXPERT-TERRITORY] revoke worker=" + ent.id() + " target=" + liveSupplyId +
					" owner=" + this.HQ.territoryMap.getOwner(liveSupply.position()));
			}
			...
			{
				// IT15.0: Farmstead-district ownership outranks a stale temporary wood
				// lease. This is the exact 14.99 failure where builders were marked
				// food-home-wait while a previous fallback order kept them walking away.
				...
				const rushFoodBackbone = this.strategyDoctrine &&
					(this.strategyDoctrine.id === "early_p1_rush" || this.strategyDoctrine.id === "late_p1_rush");
				// IT15.2: rush food civilians may use temporary wood work, but the lease may
				// never delay a newly available food slot. Retry food every update.
				if (!rushFoodBackbone && !foodRecoveryOverride && leaseResource === "wood" && Number.isFinite(leaseUntil) && gameState.ai.elapsedTime < leaseUntil)
					this.assignSafeFallback(gameState, ent, accessIndex, ["wood"]);
				else
				{
					ent.setMetadata(PlayerID, EXPERT_FALLBACK_LEASE_UNTIL, undefined);
	...
	update(gameState, queues, events)
	{
		...
		this.applyOpeningBuildingPassThrough(gameState, cc);
		...
		// IT15.1: economic food planning is own-territory only. Neutral food is not
		// appended to the working cluster graph; territorial expansion must claim it first.
		...
		const observation = observePetra(gameState, {
			...
			"flags": {
				...
				"woodForecastStatus": this.resourceForecast && this.resourceForecast.resources && this.resourceForecast.resources.wood ? this.resourceForecast.resources.wood.status : "balanced",
				"woodForecastAccessible": this.resourceForecast && this.resourceForecast.resources && this.resourceForecast.resources.wood ? this.resourceForecast.resources.wood.accessible : 0,
				"woodForecastCoverage": this.resourceForecast && this.resourceForecast.resources && this.resourceForecast.resources.wood ? this.resourceForecast.resources.wood.coverageRatio : 1,
		...
		// food reaches the <=25% transition. It cannot bypass the hard natural-food hold.
		frame = this.applyPhase2SafetyField(gameState, frame, farmCapacity, accessIndex);
		...
		if (!phasePending)
		{
			...
			{
				// IT15.8.14: economy is not a reward for finishing the attack package. The
				// first available Town food/wood technology is evaluated immediately, even
				// while an attack is assembling; Forge research continues in its own lane.
				const p2Push = this.expertP2PushInPreparation();
				let coreP2Eco = this.researchExpertP2CoreEcoTech(gameState, queues);
		...
		this.researchExpertSurplusEcoParallel(gameState, queues);
		...
		frame = this.applyForecastStorehouseInfrastructure(gameState, frame, cc, accessIndex);
		// IT14.52: worker-observed service remains the reactive safety net.
		...
		this.applyForecastBarter(gameState);
	...
	diagnose(gameState, frame, food, woodsite)
	{
		...
		aiWarn("[EXPERT-IT15.8.27] t=" + Math.round(gameState.ai.elapsedTime) +
			...
			" foodSlots=" + Math.round(this.lastImmediateFoodSlots || 0) + " bal=" + (this.lastResourceBalance && this.lastResourceBalance.active ? this.lastResourceBalance.surplus + ">" + this.lastResourceBalance.target + "@" + Number(this.lastResourceBalance.ratio || 1).toFixed(1) : "off") +
			" fc=" + (this.resourceForecast && this.resourceForecast.rankedNeeds && this.resourceForecast.rankedNeeds[0] ?
				this.resourceForecast.rankedNeeds[0].type + ":" + this.resourceForecast.rankedNeeds[0].status : "-") +
	...
	Serialize()
	{
		return {
			...
			"resourceFlowSample": this.resourceFlowSample,
			"resourceFlowIncomeEMA": { ...this.resourceFlowIncomeEMA },
			"resourceFlowSpendEMA": { ...this.resourceFlowSpendEMA },
			"resourceFlowMeasured": this.resourceFlowMeasured,
			"lastResourceForecastDiag": this.lastResourceForecastDiag,
			"lastForecastBarterAt": this.lastForecastBarterAt,
			"lastForecastStorehouseAt": this.lastForecastStorehouseAt,
			"lastForecastNaturalFoodAt": this.lastForecastNaturalFoodAt,
			...
			"lastFieldPipeline": this.lastFieldPipeline,
	...
	Deserialize(gameState, data)
	{
		...
		this.resourceFlowSample = data.resourceFlowSample;
		this.resourceFlowIncomeEMA = { food: 0, wood: 0, stone: 0, metal: 0, ...(data.resourceFlowIncomeEMA || {}) };
		this.resourceFlowSpendEMA = { food: 0, wood: 0, stone: 0, metal: 0, ...(data.resourceFlowSpendEMA || {}) };
		this.resourceFlowMeasured = !!data.resourceFlowMeasured;
		this.resourceForecast = undefined;
		this.lastResourceForecastDiag = Number.isFinite(data.lastResourceForecastDiag) ? data.lastResourceForecastDiag : -99999;
		this.lastForecastBarterAt = Number.isFinite(data.lastForecastBarterAt) ? data.lastForecastBarterAt : -99999;
		this.lastForecastStorehouseAt = Number.isFinite(data.lastForecastStorehouseAt) ? data.lastForecastStorehouseAt : -99999;
		this.lastForecastNaturalFoodAt = Number.isFinite(data.lastForecastNaturalFoodAt) ? data.lastForecastNaturalFoodAt : -99999;
		...
		this.lastFieldPipeline = Math.max(0, Number(data.lastFieldPipeline) || 0);
