...
function housingDecision(state, policy, farm = undefined, woodsite = undefined) {
  ...
  const actualFree = Math.max(0, Number(state.population.limit) - Number(state.population.used));

  // IT15.8.16: predictive/accounted headroom was still prebuilding House #3/#4 while
  // real population had 10-13 open slots. During the opening, every post-first House
  // waits for real cap pressure. Six free slots still leave the multi-builder crew and
  // live training queue time; an actual emergency remains authoritative.
  if (houses >= 1 && state.time < 360 && actualFree > 6)
    return { needed: false, maintain: false, free, trigger,
      reason: `defer opening housing until real cap pressure (${actualFree} actual free)` };
...
function fieldDemand(state, policy) {
  ...
  // IT15.8.16: six Fields authorize Barracks #2 but do not protect a food-starved
  // 50+ population economy. If natural food is gone and the bank is below one useful
  // production cycle, establish Field #7 before reserving Town.
  if (barracksPipeline >= 1 && totalNatural <= 0 && state.population.used >= 50 && state.resources.food < 200)
    desiredFields = Math.max(desiredFields, 7);
