...
export function AttackPlan(gameState, config, uniqueID, type = AttackPlan.TYPE_DEFAULT, data, deserialized)
{
	...
	{
		for (const structure of gameState.getEnemyStructures().values())
		{
			...
			{
				const sea = gameState.ai.HQ.getSeaBetweenIndices(gameState, rallyAccess, access);
				if (!sea)
				{
					if (this.target)
					{
						aiWarn("Petra: " + this.type + " " + this.name + " has an inaccessible target " +
...
AttackPlan.prototype.init = function(gameState)
{
	...
	for (const cat in this.unitStat)
	{
		const Unit = this.unitStat[cat];
		this.unit[cat] = this.unitCollection.filter(filters.byClasses(Unit.classes));
...
AttackPlan.prototype.addBuildOrder = function(gameState, name, unitStats, resetQueue)
{
	if (!this.isStarted())
	{
		...
		const Unit = this.unitStat[name];
		this.unit[name] = this.unitCollection.filter(filters.byClasses(Unit.classes));
...
AttackPlan.prototype.addSiegeUnits = function(gameState)
{
	...
	const hasTrainer = [false, false, false];
...
AttackPlan.prototype.updatePreparation = function(gameState)
{
	...
	if (gameState.getPopulationMax() - gameState.getPopulation() < 5)
	{
		...
		else	// Abort the plan so that its units will be reassigned to other plans.
		{
			if (this.Config.debug > 1)
			{
				...
				aiWarn(" attacks upcoming: raid " +
						am.upcomingAttacks[AttackPlan.TYPE_RAID].length +
				...
				aiWarn(" attacks started: raid " + am.startedAttacks[AttackPlan.TYPE_RAID].length +
	...
	for (const unitIds of gameState.ai.HQ.attackManager.bombingAttacks.values())
	{
	...
	const rallyIndex = gameState.ai.accessibility.getAccessValue(rallyPoint);
...
AttackPlan.prototype.trainMoreUnits = function(gameState)
{
	...
	{
		const special = "Plan_" + this.name + "_" + order[4];
	...
	{
		aiWarn("====================================");
		aiWarn("======== build order for plan " + this.name);
		for (const order of this.buildOrders)
		{
			const specialData = "Plan_"+this.name+"_"+order[4];
			const inTraining = gameState.countOwnQueuedEntitiesWithMetadata("special", specialData);
			const queue1 = this.queue.countQueuedUnitsWithMetadata("special", specialData);
			const queue2 = this.queueChamp.countQueuedUnitsWithMetadata("special", specialData);
			const queue3 = this.queueSiege.countQueuedUnitsWithMetadata("special", specialData);
			aiWarn(" >>> " + order[4] + " done " + order[2].length + " training " + inTraining +
				  " queue " + queue1 + " champ " + queue2 + " siege " + queue3 + " >> need " + order[3].targetSize);
		}
		aiWarn("====================================");
	}

	const firstOrder = this.buildOrders[0];
	if (firstOrder[0] < firstOrder[3].targetSize)
	{
		...
		if (queue.length() <= 5)
		{
			const template = gameState.ai.HQ.findBestTrainableUnit(gameState, firstOrder[1], firstOrder[3].interests);
			...
			{
				if (this.Config.debug > 1)
					aiWarn("attack no template found " + firstOrder[1]);
...
AttackPlan.prototype.assignUnits = function(gameState)
{
	...
	for (const ent of gameState.getOwnEntitiesByRole(undefined, true).values())
	{
	...
	for (const ent of gameState.ai.HQ.attackManager.outOfPlan.values())
	{
	...
	for (const ent of gameState.getOwnEntitiesByRole(Worker.ROLE_WORKER, true).values())
	{
		...
		{
			aiWarn("Petra problem ent without base ");
			dumpEntity(ent);
			continue;
		}
	...
	if (targetIndex != rallyIndex)
	{
		...
		for (const base of gameState.ai.HQ.baseManagers())
		{
			...
			if (!anchor || !anchor.position())
				continue;
			let dist = SquareVectorDistance(anchor.position(), this.targetPos);
		...
		else if (rallyDiff)
		{
			...
			const sea = gameState.ai.HQ.getSeaBetweenIndices(gameState, rallyIndex, targetIndex);
			if (sea)
			{
				this.overseas = sea;
...
AttackPlan.prototype.getNearestTarget = function(gameState, position, sameLand)
{
	...
	if (this.uniqueTargetId)
	{
		targets = new EntityCollection(gameState.sharedScript);
		const ent = gameState.getEntityById(this.uniqueTargetId);
		if (ent)
			targets.addEnt(ent);
	}
	else
...
AttackPlan.prototype.defaultTargetFinder = function(gameState, playerEnemy)
{
	...
	{
		for (const ent of
			gameState.getEnemyStructures(playerEnemy).filter(filters.byClass("Wonder")).values())
		{
			targets.addEnt(ent);
		}
	}
	...
	{
		for (const ent of gameState.getEnemyUnits(playerEnemy).filter(filters.byClass("Hero"))
			.values())
		{
			targets.addEnt(ent);
		}
	...
	{
		for (const ent of gameState.updatingGlobalCollection("allRelics",
			filters.byClass("Relic")).filter(relic => relic.owner() == playerEnemy).values())
		{
			targets.addEnt(ent);
		}
	}
	...
	const validTargets = gameState.getEnemyStructures(playerEnemy).filter(this.isValidTarget, this);
	targets = validTargets.filter(filters.byClass("CivCentre"));
	if (!targets.hasEntities())
		targets = validTargets.filter(filters.byClass("ConquestCritical"));
	// If there's nothing, attack anything else that's less critical
	if (!targets.hasEntities())
		targets = validTargets.filter(filters.byClass("Town"));
	if (!targets.hasEntities())
		targets = validTargets.filter(filters.byClass("Village"));
	...
	if (!targets.hasEntities())
	{
		targets = gameState.getEntities(playerEnemy).filter(filters.byClass("ConquestCritical"))
...
AttackPlan.prototype.checkTargetObstruction = function(gameState, target, position)
{
	...
	const startPos = { "x": position[0], "y": position[1] };
	const endPos = { "x": targetPos[0], "y": targetPos[1] };
	let blocker;
	const path = Engine.ComputePath(startPos, endPos, gameState.getPassabilityClassMask("default"));
	if (!path.length)
		return undefined;

	const pathPos = [path[0].x, path[0].y];
	const dist = VectorDistance(pathPos, targetPos);
	const radius = target.obstructionRadius().max;
	for (const struct of gameState.getEnemyStructures().values())
	{
		...
		const x = pathPos[0] - structPos[0] + 1.8 * (targetPos[0] - pathPos[0]) / dist;
		const y = pathPos[1] - structPos[1] + 1.8 * (targetPos[1] - pathPos[1]) / dist;
		...
		if (struct.get("Obstruction/Static"))
		{
			...
			const width = +struct.get("Obstruction/Static/@width");
			const depth = +struct.get("Obstruction/Static/@depth");
		...
		else if (struct.get("Obstruction/Obstructions"))
		{
			...
			{
				const u1 = (path[i-1].x - structPos[0]) * cosa - (path[i-1].y - structPos[1]) * sina;
				const v1 = (path[i-1].x - structPos[0]) * sina + (path[i-1].y - structPos[1]) * cosa;
				const u2 = (path[i].x - structPos[0]) * cosa - (path[i].y - structPos[1]) * sina;
				const v2 = (path[i].x - structPos[0]) * sina + (path[i].y - structPos[1]) * cosa;
				if (v1 * v2 < 0)
				{
					const u0 = (u1*v2 - u2*v1) / (v2-v1);
		...
		{
			const r = +struct.get("Obstruction/Unit/@radius");
			if (x*x + y*y < r*r)
			{
				blocker = struct;
				break;
...
AttackPlan.prototype.getPathToTarget = function(gameState, fixedRallyPoint = false)
{
	...
	const startPos = { "x": this.rallyPoint[0], "y": this.rallyPoint[1] };
...
AttackPlan.prototype.update = function(gameState, events)
{
	...
	if (this.state == "")
	{
		...
		for (const evt of events.Attacked)
		{
			...
			const ourUnit = gameState.getEntityById(evt.target);
			...
			else
			{
				...
				{	// if our unit is attacked by a siege unit, we'll send some melee units to help it.
					const collec = this.unitCollection.filter(filters.byClass("Melee"))
						.filterNearest(ourUnit.position(), 5);
					for (const ent of collec.values())
					{
						const shouldCapture = allowCapture(gameState, ent, attacker);
				...
				else
				{
					...
					const orderData = ourUnit.unitAIOrderData();
					if (orderData && orderData.length && orderData[0].target)
					{
						if (orderData[0].target === attacker.id())
							continue;
		...
		const veto = {};
		for (const target in unitTargets)
		...
		const lgth = this.unitCollUpdateArray.length < 15 || this.startingAttack ? this.unitCollUpdateArray.length : 10;
		for (let check = 0; check < lgth; check++)
		{
			...
			if (!needsUpdate)
			{
				if (!maybeUpdate)
					continue;
				const deltat = ent.unitAIState() === "INDIVIDUAL.COMBAT.APPROACHING" ? 10 : 5;
				const lastAttackPlanUpdateTime = ent.getMetadata(PlayerID, "lastAttackPlanUpdateTime");
			...
			else
			{
				...
				const mUnit = enemyUnits.filter(enemy =>
				{
					...
					if (veto[enemy.id()] &&
						SquareVectorDistance(this.targetPos, ent.position()) > 2500)
					{
						return false;
					}
				...
				if (mUnit.length)
				{
					mUnit.sort((unitA, unitB) =>
					{
						...
						if (ent.counters(unitB))
							valb += 100;
						const distA = unitA.getMetadata(PlayerID, "distance");
						const distB = unitB.getMetadata(PlayerID, "distance");
						if (distA && distB)
						{
							vala -= distA;
							valb -= distB;
...
AttackPlan.prototype.UpdateWalking = function(gameState, events)
{
	...
	if (SquareVectorDistance(this.position, this.position5TurnsAgo) < 10 &&
		this.path.length > 0 && gameState.ai.playedTurn % 5 === 0)
	{
		...
		this.path.length > 0)
	{
		if (!this.path[0][0] || !this.path[0][1])
			aiWarn("Start: Problem with path " + uneval(this.path));
		// We're stuck, presumably. Check if there are no walls just close to us.
		for (const ent of gameState.getEnemyStructures().filter(filters.byClass(["Palisade", "Wall"]))
			.values())
		{
			if (SquareVectorDistance(this.position, ent.position()) > 800)
				continue;
			const enemyClass = ent.hasClass("Wall") ? "Wall" : "Palisade";
			// there are walls, so check if we can attack
			if (this.unitCollection.filter(filters.byCanAttackClass(enemyClass)).hasEntities())
			{
				if (this.Config.debug > 1)
				{
					aiWarn("Attack Plan " + this.type + " " + this.name +
						" has met walls and is not happy.");
				}
				this.state = AttackPlan.STATE_ARRIVED;
				return true;
			}
	...
	if (SquareVectorDistance(this.position, this.targetPos) < 10000)
	{
		if (this.Config.debug > 1)
			aiWarn("Attack Plan " + this.type + " " + this.name + " has arrived to destination.");
		this.state = AttackPlan.STATE_ARRIVED;
		return true;
	}
	else if (this.path.length && SquareVectorDistance(this.position, this.path[0]) < 1600)
	{
		this.path.shift();
		if (this.path.length)
			this.unitCollection.moveToRange(this.path[0][0], this.path[0][1], 0, 15);
...
AttackPlan.prototype.UpdateTarget = function(gameState)
{
	...
	if (!this.target || !gameState.getEntityById(this.target.id()))
	{
		if (this.Config.debug > 1)
		{
			aiWarn("Seems like our target for plan " + this.name +
				" has been destroyed or captured. Switching.");
		}
		const accessIndex = this.getAttackAccess(gameState);
		...
		if (!this.target)
		{
			...
			const attackManager = gameState.ai.HQ.attackManager;
			for (const attackType in attackManager.startedAttacks)
			{
				for (const attack of attackManager.startedAttacks[attackType])
				{
			...
			if (!this.target)
			{
				...
				if (!this.target)
				{
					if (this.Config.debug > 1)
					{
						aiWarn("No new target found. Remaining units " +
		...
		if (this.isStarted())
		{
			...
			for (const base of gameState.ai.HQ.baseManagers())
			{
				...
				if (getLandAccess(gameState, base.anchor) != access)
					continue;
				const newdist = SquareVectorDistance(this.position, base.anchor.position());
				if (newdist > dist)
					continue;
				dist = newdist;
				rallyPoint = base.anchor.position();
...
AttackPlan.prototype.checkEvents = function(gameState, events)
{
	...
	for (const evt of events.OwnershipChanged)	// capture event
...
AttackPlan.prototype.debugAttack = function()
{
	...
	{
		const Unit = this.unitStat[unitCat];
		aiWarn(unitCat + " num=" + this.unit[unitCat].length + " min=" + Unit.minSize + " need=" +
			Unit.targetSize);
	}
	aiWarn("------------------------------");
};
